...
@Injectable()
export class ProductionVariancePostingRule implements PostingRule<ProductionVarianceEntity> {
  constructor(
    @InjectRepository(ProductionVarianceEntity)
    private readonly sourceRepository: Repository<ProductionVarianceEntity>,
  ...
  async build(
    ...
    const varianceAccount = this.requireAccount(
    ...
    const signedVariance = this.round(Number(source.totalVariance));

    if (Math.abs(signedVariance) <= 0.009) {
      throw new ConflictException('Production variance amount is zero.');
    }

    const amount = Math.abs(signedVariance);

    const unfavorable = signedVariance > 0;
    ...
    return {
      ...
      referenceNumber: source.id,
      narration: 'Production variance settlement',
      ...
      lines: unfavorable
        ? [
            {
              ...
              debit: amount,
              credit: 0,
              description: 'Unfavorable manufacturing variance',
              partyType: null,
              partyId: null,
              costCenter: null,
            },
            {
              ...
              debit: 0,
              credit: amount,
              description: 'WIP variance settlement',
              partyType: null,
              partyId: null,
              costCenter: null,
            },
        ...
        : [
            ...
            },
            {
              ...
              debit: 0,
              credit: amount,
              description: 'Favorable manufacturing variance',
              partyType: null,
              partyId: null,
              costCenter: null,
            },
