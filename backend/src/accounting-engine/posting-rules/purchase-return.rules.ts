...
type PurchaseReturnAccountingSettings = AccountingSettingsEntity & {
...
@Injectable()
export class PurchaseReturnPostingRule implements PostingRule<PurchaseReturn> {
  constructor(
  ...
  async load(sourceId: string, companyId: string): Promise<PurchaseReturn> {
  ...
  async build(
    ...
    const purchaseReturnCreditAccountId = this.requireAccount(
      settings.purchaseReturnsAccountId ?? settings.inventoryAccountId,
      'Purchase Returns or Inventory',
    );

    const subtotalAfterDiscount = this.round(
      Number(purchaseReturn.subtotal) - Number(purchaseReturn.discountTotal),
    );

    const taxTotal = this.round(Number(purchaseReturn.taxTotal));

    const grandTotal = this.round(Number(purchaseReturn.grandTotal));

    if (subtotalAfterDiscount < 0 || taxTotal < 0 || grandTotal <= 0) {
      throw new ConflictException(
        'Purchase return contains invalid accounting totals.',
      );
    }

    const expectedGrandTotal = this.round(subtotalAfterDiscount + taxTotal);

    if (Math.abs(expectedGrandTotal - grandTotal) > 0.009) {
      throw new ConflictException('Purchase return totals are not balanced.');
    }
    ...
    const lines: PostingDocument['lines'] = [
      {
        ...
        debit: grandTotal,
        credit: 0,
        description: `Accounts payable reduction for purchase return ${purchaseReturn.returnNumber}`,
        partyType: 'supplier',
      ...
      },
      {
        ...
        description: `Inventory or purchase return value for ${purchaseReturn.returnNumber}`,
        partyType: null,
        partyId: null,
        costCenter: null,
      },
    ...
    if (taxTotal > 0) {
      lines.push({
        ...
        debit: 0,
        credit: taxTotal,
        description: `Input tax reversal for purchase return ${purchaseReturn.returnNumber}`,
        partyType: null,
        partyId: null,
        costCenter: null,
      });
    ...
    return {
      companyId,
      sourceType: JournalEntrySourceType.PURCHASE_RETURN,
      sourceId: purchaseReturn.id,
      entryDate: purchaseReturn.returnDate,
      referenceNumber: purchaseReturn.returnNumber,
      currency: purchaseReturn.currency,
      narration: `Automatic posting for purchase return ${purchaseReturn.returnNumber}`,
      lines,
    };
  }
