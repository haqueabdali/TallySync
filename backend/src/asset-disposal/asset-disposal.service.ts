...
export class AssetDisposalService {
  constructor(
    ...
    @InjectRepository(AssetDisposalEntity)
    private readonly disposalRepo: Repository<AssetDisposalEntity>,
  ...
  async create(
    ...
    if (
      ...
      throw new ConflictException(
        'Only active, inactive, or fully depreciated assets can be disposed.',
      );
    if (dto.disposalDate < asset.acquisitionDate)
      throw new BadRequestException(
        'Disposal date cannot be before acquisition date.',
      );
    if (dto.disposalProceeds > 0 && !dto.proceedsAccountId)
      throw new BadRequestException(
        'Proceeds account is required when disposal proceeds are greater than zero.',
      );
    ...
    const existing = await this.disposalRepo.exists({
      where: { assetId: asset.id },
    });
    if (existing)
      throw new ConflictException('A disposal already exists for this asset.');
    const accumulated = await this.getPostedDepreciation(
      companyId,
      asset.id,
      dto.disposalDate,
    );
    ...
    return this.disposalRepo.save(
      this.disposalRepo.create({
        ...
        disposalProceeds: this.money(dto.disposalProceeds),
        accumulatedDepreciation: this.money(accumulated),
        netBookValue: this.money(netBookValue),
        gainAmount: this.money(gain),
        lossAmount: this.money(loss),
        proceedsAccountId: dto.proceedsAccountId ?? null,
        gainAccountId: dto.gainAccountId,
        lossAccountId: dto.lossAccountId,
        status: AssetDisposalStatus.DRAFT,
        journalEntryId: null,
  ...
  async post(
    ...
    const latestAccumulated = await this.getPostedDepreciation(
      companyId,
      disposal.assetId,
      disposal.disposalDate,
    );
    if (
      this.round(latestAccumulated) !==
      this.round(Number(disposal.accumulatedDepreciation))
    )
      throw new ConflictException(
        'Depreciation changed after the disposal draft was created. Recreate the disposal.',
      );
    ...
    if (proceeds > 0 && disposal.proceedsAccountId)
      lines.push({
        accountId: disposal.proceedsAccountId,
        debit: proceeds,
        credit: 0,
        description: `Asset disposal proceeds - ${disposal.asset.assetNumber}`,
      });
    if (accumulated > 0)
      lines.push({
        accountId: disposal.asset.category.accumulatedDepreciationAccountId,
        debit: accumulated,
        credit: 0,
        description: `Remove accumulated depreciation - ${disposal.asset.assetNumber}`,
      });
    if (loss > 0)
      lines.push({
        accountId: disposal.lossAccountId,
        debit: loss,
        credit: 0,
        description: `Loss on asset disposal - ${disposal.asset.assetNumber}`,
      });
    lines.push({
      accountId: disposal.asset.category.assetAccountId,
      debit: 0,
      credit: cost,
      description: `Remove asset cost - ${disposal.asset.assetNumber}`,
    });
    if (gain > 0)
      lines.push({
        accountId: disposal.gainAccountId,
        debit: 0,
        credit: gain,
        description: `Gain on asset disposal - ${disposal.asset.assetNumber}`,
      });
    const journal = await this.journalEntriesService.create(
      {
        ...
        referenceNumber: `DISP-${disposal.asset.assetNumber}`,
        narration: `Disposal of fixed asset ${disposal.asset.assetNumber}`,
        lines,
      },
      companyId,
      userId,
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      locked.status = AssetDisposalStatus.POSTED;
      locked.journalEntryId = journal.id;
      locked.postedBy = userId;
      locked.postedAt = new Date();
      await repo.save(locked);
      await manager
        .getRepository(FixedAssetEntity)
        .update(
          { id: locked.assetId, companyId },
          {
            status: AssetStatus.DISPOSED,
            disposalDate: locked.disposalDate,
            disposalProceeds: locked.disposalProceeds,
            updatedBy: userId,
          },
        );
    });
  ...
  async list(companyId: string, filter: AssetDisposalFilterDto) {
    const [data, total] = await this.disposalRepo.findAndCount({
      where: {
        companyId,
      ...
      order: { disposalDate: 'DESC', createdAt: 'DESC' },
  ...
  async get(companyId: string, id: string): Promise<AssetDisposalEntity> {
    const entity = await this.disposalRepo.findOne({
      where: { id, companyId },
      relations: { asset: { category: true } },
    });
  ...
  private async getPostedDepreciation(
    companyId: string,
    assetId: string,
    disposalDate: string,
  ): Promise<number> {
    const row = await this.dataSource
      .getRepository(DepreciationEntryEntity)
      .createQueryBuilder('entry')
      .innerJoin('entry.run', 'run')
      .select('COALESCE(SUM(entry.depreciation_amount), 0)', 'amount')
      .where('entry.company_id = :companyId', { companyId })
      .andWhere('entry.asset_id = :assetId', { assetId })
      .andWhere('run.status = :status', {
        status: DepreciationRunStatus.POSTED,
      })
      .andWhere('run.period_end <= :disposalDate', { disposalDate })
  ...
  private async validateAccounts(
    companyId: string,
    proceedsId: string | undefined,
    gainId: string,
    lossId: string,
  ): Promise<void> {
    const ids = [gainId, lossId, ...(proceedsId ? [proceedsId] : [])];
    if (new Set(ids).size !== ids.length)
      throw new BadRequestException('Disposal accounts must be different.');
    const accounts = await this.accountRepo.findBy(
      ids.map((id) => ({ id, companyId })),
    );
    if (accounts.length !== ids.length)
      throw new BadRequestException(
        'One or more disposal accounts are invalid.',
      );
    ...
    if (accounts.find((x) => x.id === gainId)?.type !== AccountType.INCOME)
      throw new BadRequestException('Gain account must be an income account.');
    if (accounts.find((x) => x.id === lossId)?.type !== AccountType.EXPENSE)
      throw new BadRequestException('Loss account must be an expense account.');
    if (
      proceedsId &&
      accounts.find((x) => x.id === proceedsId)?.type !== AccountType.ASSET
    )
      throw new BadRequestException(
        'Proceeds account must be an asset account.',
      );
  }
