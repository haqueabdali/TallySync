...
export class AssetManagementService {
  constructor(
    ...
    private readonly categoryRepo: Repository<AssetCategoryEntity>,
    @InjectRepository(FixedAssetEntity)
    private readonly assetRepo: Repository<FixedAssetEntity>,
  ...
  async createCategory(
    ...
    await this.validateCategoryAccounts(
      companyId,
      dto.assetAccountId,
      dto.accumulatedDepreciationAccountId,
      dto.depreciationExpenseAccountId,
    );
    this.validateDecliningRate(
      dto.depreciationMethod,
      dto.decliningBalanceRate,
    );
    const existing = await this.categoryRepo.exists({
    ...
    const entity = this.categoryRepo.create({
      ...
      decliningBalanceRate:
        dto.decliningBalanceRate === undefined
          ? null
          : dto.decliningBalanceRate.toFixed(4),
  ...
  async updateCategory(
    ...
    const expenseId =
      dto.depreciationExpenseAccountId ?? entity.depreciationExpenseAccountId;
    await this.validateCategoryAccounts(
      companyId,
      assetAccountId,
      accumulatedId,
    ...
    const rate =
      dto.decliningBalanceRate ??
      (entity.decliningBalanceRate === null
        ? undefined
        : Number(entity.decliningBalanceRate));
    ...
    if (dto.description !== undefined)
      entity.description = this.optional(dto.description);
    entity.assetAccountId = assetAccountId;
    entity.accumulatedDepreciationAccountId = accumulatedId;
    entity.depreciationExpenseAccountId = expenseId;
    ...
    entity.decliningBalanceRate =
      method === DepreciationMethod.DECLINING_BALANCE && rate !== undefined
        ? rate.toFixed(4)
        : null;
  ...
  async removeCategory(
    ...
    const used = await this.assetRepo.exists({
      where: { companyId, categoryId: id },
    });
    if (used)
      throw new ConflictException(
        'Asset category is used by one or more assets',
      );
  ...
  ): Promise<FixedAssetEntity> {
    const category = await this.getCategory(companyId, dto.categoryId);
    if (!category.isActive)
      throw new BadRequestException('Asset category is inactive');
    this.validateAssetValues(
      dto.acquisitionCost,
      dto.residualValue ?? 0,
      dto.acquisitionDate,
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      return repo.save(
        repo.create({
          ...
          acquisitionCost: dto.acquisitionCost.toFixed(4),
          residualValue: (dto.residualValue ?? 0).toFixed(4),
          usefulLifeMonths:
          ...
          custodian: this.optional(dto.custodian),
  ...
  async listAssets(
    ...
    if (filter.search?.trim()) {
      ...
      qb.andWhere(
        new Brackets((b) =>
          b
            .where('asset.asset_number ILIKE :search', { search })
            .orWhere('asset.name ILIKE :search', { search })
            .orWhere('asset.serial_number ILIKE :search', { search }),
        ),
      );
    }
    ...
    if (filter.isActiveCategory !== undefined)
      qb.andWhere('category.is_active = :isActiveCategory', {
        isActiveCategory: filter.isActiveCategory,
      });
  ...
  async updateAsset(
    ...
    if (asset.status !== AssetStatus.DRAFT)
      throw new ConflictException('Only draft assets can be edited');
    const category = dto.categoryId
      ? await this.getCategory(companyId, dto.categoryId)
      : asset.category;
    if (!category.isActive)
      throw new BadRequestException('Asset category is inactive');
    const acquisitionCost =
      dto.acquisitionCost ?? Number(asset.acquisitionCost);
    const residualValue = dto.residualValue ?? Number(asset.residualValue);
    const acquisitionDate = dto.acquisitionDate ?? asset.acquisitionDate;
    const depreciationStartDate =
      dto.depreciationStartDate ?? asset.depreciationStartDate;
    this.validateAssetValues(
      acquisitionCost,
      residualValue,
      acquisitionDate,
    ...
    if (dto.description !== undefined)
      asset.description = this.optional(dto.description);
    asset.categoryId = category.id;
    asset.acquisitionDate = acquisitionDate;
    asset.depreciationStartDate = depreciationStartDate;
    asset.acquisitionCost = acquisitionCost.toFixed(4);
    asset.residualValue = residualValue.toFixed(4);
    if (dto.usefulLifeMonths !== undefined)
      asset.usefulLifeMonths = dto.usefulLifeMonths;
    if (dto.serialNumber !== undefined)
      asset.serialNumber = this.optional(dto.serialNumber);
    if (dto.location !== undefined)
      asset.location = this.optional(dto.location);
    if (dto.custodian !== undefined)
      asset.custodian = this.optional(dto.custodian);
  ...
  async activateAsset(
    ...
    if (
      ...
      throw new ConflictException(
        'Only draft or inactive assets can be activated',
      );
    asset.status = AssetStatus.ACTIVE;
    asset.activationDate =
      asset.activationDate ?? new Date().toISOString().slice(0, 10);
    asset.updatedBy = userId;
    return this.assetRepo.save(asset);
  }
  ...
  async deactivateAsset(
    ...
    if (asset.status !== AssetStatus.ACTIVE)
      throw new ConflictException('Only active assets can be deactivated');
    asset.status = AssetStatus.INACTIVE;
    asset.updatedBy = userId;
    return this.assetRepo.save(asset);
  }
  ...
  async removeAsset(
    ...
    const asset = await this.getAsset(companyId, id);
    if (asset.status !== AssetStatus.DRAFT)
      throw new ConflictException('Only draft assets can be deleted');
  ...
  private async getCategory(
    ...
    const e = await this.categoryRepo.findOne({ where: { companyId, id } });
    if (!e) throw new NotFoundException('Asset category not found');
    return e;
  }
  private async validateCategoryAccounts(
    companyId: string,
    assetId: string,
    accumulatedId: string,
    expenseId: string,
  ): Promise<void> {
    const ids = [assetId, accumulatedId, expenseId];
    if (new Set(ids).size !== 3)
      throw new BadRequestException(
        'Asset category accounts must be different',
      );
    const accounts = await this.accountRepo.findBy(
      ids.map((id) => ({ id, companyId })),
    );
    if (accounts.length !== 3)
      throw new BadRequestException(
        'One or more asset category accounts are invalid',
      );
    for (const account of accounts)
      if (account.status !== AccountStatus.ACTIVE || account.isGroup)
        throw new BadRequestException(
          `Account ${account.code} must be an active posting account`,
        );
    const assetAccount = accounts.find((x) => x.id === assetId);
    const accumulated = accounts.find((x) => x.id === accumulatedId);
    const expense = accounts.find((x) => x.id === expenseId);
    if (
      assetAccount?.type !== AccountType.ASSET ||
      accumulated?.type !== AccountType.ASSET
    )
      throw new BadRequestException(
        'Asset and accumulated depreciation accounts must be asset accounts',
      );
    if (expense?.type !== AccountType.EXPENSE)
      throw new BadRequestException(
        'Depreciation expense account must be an expense account',
      );
  }
  private validateDecliningRate(
    method: DepreciationMethod,
    rate?: number,
  ): void {
    if (method === DepreciationMethod.DECLINING_BALANCE && rate === undefined)
      throw new BadRequestException('Declining balance rate is required');
  }
  private validateAssetValues(
    cost: number,
    residual: number,
    ...
    if (depreciationStartDate < acquisitionDate)
      throw new BadRequestException(
  ...
  private async generateAssetNumber(
    companyId: string,
    repo: Repository<FixedAssetEntity>,
  ): Promise<string> {
    const row = await repo
      .createQueryBuilder('a')
      .withDeleted()
      .select('a.asset_number', 'number')
      .where('a.company_id = :companyId', { companyId })
      .andWhere("a.asset_number ~ '^FA-[0-9]+$'")
      .orderBy('CAST(SUBSTRING(a.asset_number FROM 4) AS INTEGER)', 'DESC')
      .getRawOne<{ number?: string }>();
    const next = row?.number ? Number(row.number.slice(3)) + 1 : 1;
