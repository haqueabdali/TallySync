...
@RequireLicenseFeature(LicensedFeature.BANK_RECONCILIATION)
...
export class BankReconciliationController {
  ...
  @Get() @ApiOkResponse() list(
    @Req() request: AuthenticatedRequest,
    @Query() filter: BankReconciliationFilterDto,
  ) {
  ...
  @Get(':id') @ApiOkResponse() get(
    ...
    return this.service.get(request.user.companyId, id);
  }
  @Post(':id/lines/:lineId/matches')
  @ApiOperation({
    summary: 'Match a statement line to a posted bank journal line',
  })
  match(
    @Req() request: AuthenticatedRequest,
    @Param('id', ParseUUIDPipe) id: string,
  ...
  @Delete(':id/lines/:lineId/matches/:matchId') @ApiOkResponse() unmatch(
    @Req() request: AuthenticatedRequest,
    @Param('id', ParseUUIDPipe) id: string,
    @Param('lineId', ParseUUIDPipe) lineId: string,
    @Param('matchId', ParseUUIDPipe) matchId: string,
  ) {
    return this.service.unmatch(request.user.companyId, id, lineId, matchId);
  }
  @Post(':id/reconcile') @ApiOkResponse() reconcile(
    @Req() request: AuthenticatedRequest,
    @Param('id', ParseUUIDPipe) id: string,
  ) {
    return this.service.reconcile(request.user.companyId, request.user.id, id);
  }
}
