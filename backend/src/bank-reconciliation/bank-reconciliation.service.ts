...
export class BankReconciliationService {
  constructor(
    ...
    private readonly reconciliationRepository: Repository<BankReconciliationEntity>,
  ...
  async create(
    ...
    if (dto.statementStartDate > dto.statementEndDate)
      throw new BadRequestException(
        'Statement start date cannot be after statement end date',
      );
    await this.validateBankAccount(companyId, dto.bankAccountId);
    const duplicate = await this.reconciliationRepository.findOne({
      where: { companyId, statementReference: dto.statementReference.trim() },
    });
    if (duplicate)
      throw new ConflictException('Statement reference already exists');
    const movement = this.round(
      dto.lines.reduce((sum, line) => sum + line.amount, 0),
    );
    const expectedClosing = this.round(dto.openingBalance + movement);
    if (!this.equal(expectedClosing, dto.closingBalance))
      throw new BadRequestException(
        `Statement balance mismatch: opening balance plus transactions equals ${expectedClosing.toFixed(2)}`,
      );
    return this.dataSource.transaction(async (manager) => {
      const reconciliationRepository = manager.getRepository(
      ...
      reconciliation.lines = await lineRepository.save(
        dto.lines.map((line) =>
  ...
  async list(
    companyId: string,
    filter: BankReconciliationFilterDto,
  ): Promise<{
    data: BankReconciliationEntity[];
    ...
    if (filter.bankAccountId)
      query.andWhere('reconciliation.bank_account_id = :bankAccountId', {
        bankAccountId: filter.bankAccountId,
      });
    if (filter.status)
      query.andWhere('reconciliation.status = :status', {
        status: filter.status,
      });
    query
      .orderBy('reconciliation.statement_end_date', 'DESC')
      .addOrderBy('reconciliation.created_at', 'DESC')
      .skip((page - 1) * limit)
      .take(limit);
  ...
  async get(companyId: string, id: string): Promise<BankReconciliationEntity> {
    const reconciliation = await this.reconciliationRepository
      .createQueryBuilder('reconciliation')
      .leftJoinAndSelect('reconciliation.bankAccount', 'bankAccount')
  ...
  async match(
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      const lineRepository = manager.getRepository(BankStatementLineEntity);
      const matchRepository = manager.getRepository(
        BankReconciliationMatchEntity,
      );
      const journalLineRepository = manager.getRepository(
      ...
      if (
        ...
        throw new ConflictException(
          'Finalized reconciliation cannot be changed',
        );
      const line = await lineRepository.findOne({
        where: { id: statementLineId, reconciliationId },
        lock: { mode: 'pessimistic_write' },
      });
      if (!line) throw new NotFoundException('Bank statement line not found');
      const journalLine = await journalLineRepository
        ...
        .getOne();
      if (!journalLine)
        throw new NotFoundException('Posted journal entry line not found');
      if (journalLine.accountId !== reconciliation.bankAccountId)
        throw new BadRequestException(
          'Journal entry line does not use the selected bank account',
        );
      const duplicate = await matchRepository.findOne({
        where: { statementLineId, journalEntryLineId: dto.journalEntryLineId },
      });
      if (duplicate)
        throw new ConflictException(
          'Journal entry line is already matched to this statement line',
        );
      const statementRemaining = this.round(
        Math.abs(line.amount) - line.matchedAmount,
      );
      if (dto.amount > statementRemaining + 0.005)
        throw new BadRequestException(
          'Match amount exceeds the unmatched statement amount',
        );
      const journalAmount = this.round(
        Math.abs(journalLine.debit - journalLine.credit),
      );
      const previouslyMatchedResult = await matchRepository
        .createQueryBuilder('match')
        .select('COALESCE(SUM(match.amount), 0)', 'total')
        .where('match.journal_entry_line_id = :journalEntryLineId', {
          journalEntryLineId: journalLine.id,
        })
        .getRawOne<{ total: string }>();
      const journalRemaining = this.round(
        journalAmount - Number(previouslyMatchedResult?.total ?? 0),
      );
      if (dto.amount > journalRemaining + 0.005)
        throw new BadRequestException(
          'Match amount exceeds the unmatched journal amount',
        );
      const statementDirection = Math.sign(line.amount);
      const journalDirection = Math.sign(
        journalLine.debit - journalLine.credit,
      );
      if (statementDirection !== journalDirection)
        throw new BadRequestException(
          'Statement and journal line directions do not match',
        );
  ...
  async unmatch(
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      const match = await matchRepository.findOne({
        where: { id: matchId, statementLineId },
      });
      if (!match) throw new NotFoundException('Reconciliation match not found');
      await matchRepository.remove(match);
      line.matchedAmount = this.round(
        Math.max(0, line.matchedAmount - match.amount),
      );
      line.status = this.lineStatus(line.amount, line.matchedAmount);
      await lineRepository.save(line);
    });
  ...
  async reconcile(
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      const reconciliation = await repository.findOne({
        where: { id, companyId },
      ...
      if (reconciliation.status === BankReconciliationStatus.CANCELLED)
        throw new ConflictException(
          'Cancelled reconciliation cannot be reconciled',
        );
      const unmatched = reconciliation.lines.filter(
        (line) => line.status !== BankStatementLineStatus.MATCHED,
      );
      if (unmatched.length > 0)
        throw new BadRequestException(
          `${unmatched.length} statement line(s) are not fully matched`,
        );
      reconciliation.status = BankReconciliationStatus.RECONCILED;
  ...
  private async validateBankAccount(
    ...
    if (account.status !== AccountStatus.ACTIVE || account.isGroup)
      throw new BadRequestException(
        'Bank account must be an active posting account',
      );
  }
  private lineStatus(
    amount: number,
    ...
    return this.equal(Math.abs(amount), matchedAmount)
      ? BankStatementLineStatus.MATCHED
      : BankStatementLineStatus.PARTIALLY_MATCHED;
  }
