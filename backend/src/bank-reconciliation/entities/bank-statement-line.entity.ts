...
@Index(
  'UQ_bank_statement_lines_external',
  ['reconciliationId', 'externalTransactionId'],
...
export class BankStatementLineEntity {
  ...
  @ManyToOne(
    ...
    (reconciliation) => reconciliation.lines,
  ...
  @Column({ name: 'value_date', type: 'date', nullable: true }) valueDate!:
    string | null;
  @Column({
  ...
  @OneToMany(
    ...
    (match) => match.statementLine,
    { eager: true },
  )
  matches!: BankReconciliationMatchEntity[];
