...
describe('CostingVarianceService', () => {
  let service: CostingVarianceService;

  const analysisRepository = {
    findOne: jest.fn(),
    find: jest.fn().mockResolvedValue([]),
    save: jest.fn(async (value: ProductionCostAnalysisEntity) => value),
    createQueryBuilder: jest.fn(),
  };

  const materialRepository = {
    create: jest.fn(
      (value: Partial<ProductionCostMaterialLineEntity>) => value,
    ),
    save: jest.fn(async (value: ProductionCostMaterialLineEntity[]) => value),
    delete: jest.fn(),
  };
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getRepositoryToken(ProductionCostAnalysisEntity),
          useValue: analysisRepository,
        },
        {
          provide: getRepositoryToken(ProductionCostMaterialLineEntity),
    ...
    service = module.get(CostingVarianceService);
  });
  ...
  it('prevents duplicate active production reference', async () => {
    ...
    await expect(
      service.create('company', 'user', {
        productionReferenceId: '11111111-1111-4111-8111-111111111111',
  ...
  it('requires actual output before finalization', async () => {
    ...
    await expect(
      service.finalize('company', 'user', 'analysis'),
    ).rejects.toThrow(ConflictException);
  });
});
