...
export class CreateAssetManagement1787604000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."asset_categories_depreciation_method_enum" AS ENUM('straight_line','declining_balance')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."fixed_assets_status_enum" AS ENUM('draft','active','fully_depreciated','disposed','inactive')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE UNIQUE INDEX "UQ_fixed_assets_company_number" ON "fixed_assets" ("company_id", "asset_number") WHERE "deleted_at" IS NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE "fixed_assets" ADD CONSTRAINT "FK_fixed_assets_category" FOREIGN KEY ("category_id") REFERENCES "asset_categories"("id") ON DELETE RESTRICT`,
    );
  }
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "fixed_assets" DROP CONSTRAINT "FK_fixed_assets_category"`,
    );
    await queryRunner.query(`DROP TABLE "fixed_assets"`);
    await queryRunner.query(
      `ALTER TABLE "asset_categories" DROP CONSTRAINT "FK_asset_category_expense_account"`,
    );
    await queryRunner.query(
      `ALTER TABLE "asset_categories" DROP CONSTRAINT "FK_asset_category_accumulated_account"`,
    );
    await queryRunner.query(
      `ALTER TABLE "asset_categories" DROP CONSTRAINT "FK_asset_category_asset_account"`,
    );
    await queryRunner.query(`DROP TABLE "asset_categories"`);
    await queryRunner.query(`DROP TYPE "public"."fixed_assets_status_enum"`);
    await queryRunner.query(
      `DROP TYPE "public"."asset_categories_depreciation_method_enum"`,
    );
  }
}
