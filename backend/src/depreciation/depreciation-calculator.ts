...
export function calculateDepreciation(
  ...
  const maximumDepreciation = round(
    input.acquisitionCost - input.residualValue,
  );
  const remaining = round(
    maximumDepreciation - input.openingAccumulatedDepreciation,
  );
  if (remaining <= 0 || input.monthsToDepreciate <= 0)
    return {
      amount: 0,
      closingAccumulatedDepreciation: round(
        input.openingAccumulatedDepreciation,
      ),
      closingNetBookValue: round(
        input.acquisitionCost - input.openingAccumulatedDepreciation,
      ),
    };
  let amount: number;
  if (input.method === DepreciationMethod.STRAIGHT_LINE) {
    amount =
      (maximumDepreciation / input.usefulLifeMonths) * input.monthsToDepreciate;
  } else {
    if (input.decliningBalanceRate === null || input.decliningBalanceRate <= 0)
      throw new BadRequestException(
        'Declining-balance rate must be configured.',
      );
    let bookValue =
      input.acquisitionCost - input.openingAccumulatedDepreciation;
    amount = 0;
  ...
  const closingAccumulatedDepreciation = round(
    input.openingAccumulatedDepreciation + amount,
  );
  return {
    amount,
    closingAccumulatedDepreciation,
    closingNetBookValue: round(
      input.acquisitionCost - closingAccumulatedDepreciation,
    ),
  };
}
