...
export class DepreciationService {
  constructor(
    ...
    @InjectRepository(DepreciationRunEntity)
    private readonly runRepository: Repository<DepreciationRunEntity>,
    @InjectRepository(DepreciationEntryEntity)
    private readonly entryRepository: Repository<DepreciationEntryEntity>,
    @InjectRepository(FixedAssetEntity)
    private readonly assetRepository: Repository<FixedAssetEntity>,
  ...
  async createRun(
    companyId: string,
    userId: string,
    dto: CreateDepreciationRunDto,
  ): Promise<DepreciationRunEntity> {
    if (dto.postingDate < dto.periodEnd)
      throw new BadRequestException(
        'Posting date cannot be before period end.',
      );
    const existing = await this.runRepository.findOne({
      where: { companyId, periodEnd: dto.periodEnd },
    });
    if (existing)
      throw new ConflictException(
        'A depreciation run already exists for this period.',
      );
    const calculatedEntries = await this.calculateEntries(
      companyId,
      dto.periodEnd,
    );
    if (calculatedEntries.length === 0)
      throw new BadRequestException(
        'No depreciation is due for the selected period.',
      );
    return this.dataSource.transaction(async (manager) => {
      ...
      const entries = calculatedEntries.map((entry) =>
        entryRepo.create({ ...entry, runId: run.id, companyId }),
      );
  ...
  async postRun(
    ...
    if (run.entries.length === 0)
      throw new BadRequestException('Depreciation run has no entries.');
    const grouped = new Map<
      string,
      { expenseAccountId: string; accumulatedAccountId: string; amount: number }
    >();
    for (const entry of run.entries) {
      ...
      const current = grouped.get(key) ?? {
        ...
        accumulatedAccountId: entry.accumulatedDepreciationAccountId,
        amount: 0,
      };
      current.amount = this.round(
        current.amount + Number(entry.depreciationAmount),
      );
    ...
    const lines = Array.from(grouped.values()).flatMap((group) => [
      {
        ...
        debit: group.amount,
        credit: 0,
        description: `Depreciation expense through ${run.periodEnd}`,
      },
      {
        accountId: group.accumulatedAccountId,
        debit: 0,
        credit: group.amount,
        description: `Accumulated depreciation through ${run.periodEnd}`,
      },
    ]);
    const journal = await this.journalEntriesService.create(
      {
        ...
        sourceId: run.id,
        referenceNumber: `DEP-${run.periodEnd}`,
        narration: `Depreciation run through ${run.periodEnd}`,
        lines,
      },
      companyId,
      userId,
    ...
    await this.dataSource.transaction(async (manager) => {
      const lockedRun = await manager
        .getRepository(DepreciationRunEntity)
        .findOne({
          where: { id, companyId },
      ...
      for (const entry of run.entries) {
        if (
          Number(entry.closingNetBookValue) <= Number(entry.asset.residualValue)
        ) {
          await assetRepo.update(
            { id: entry.assetId, companyId },
            { status: AssetStatus.FULLY_DEPRECIATED, updatedBy: userId },
          );
        }
      }
    });
  ...
  private async calculateEntries(
    ...
    Array<Partial<DepreciationEntryEntity> & { asset: FixedAssetEntity }>
  > {
    const assets = await this.assetRepository.find({
      where: {
        companyId,
        status: AssetStatus.ACTIVE,
        depreciationStartDate: LessThanOrEqual(periodEnd),
      },
      relations: { category: true },
      order: { assetNumber: 'ASC' },
    });
    const result: Array<
      Partial<DepreciationEntryEntity> & { asset: FixedAssetEntity }
    > = [];
    for (const asset of assets) {
      const prior = await this.entryRepository
        .createQueryBuilder('entry')
        .innerJoin('entry.run', 'run')
        .select('COALESCE(SUM(entry.depreciation_amount), 0)', 'amount')
        .addSelect('MAX(run.period_end)', 'lastPeriodEnd')
        .where('entry.company_id = :companyId', { companyId })
        .andWhere('entry.asset_id = :assetId', { assetId: asset.id })
        .andWhere('run.status = :status', {
          status: DepreciationRunStatus.POSTED,
        })
        .andWhere('run.period_end <= :periodEnd', { periodEnd })
        .getRawOne<{ amount: string; lastPeriodEnd: string | null }>();
      const openingAccumulated = Number(prior?.amount ?? 0);
      const fromDate = prior?.lastPeriodEnd
        ? this.nextMonth(prior.lastPeriodEnd)
      ...
      const calculation = calculateDepreciation({
        ...
        decliningBalanceRate:
          asset.category.decliningBalanceRate === null
            ? null
            : Number(asset.category.decliningBalanceRate),
        openingAccumulatedDepreciation: openingAccumulated,
        monthsToDepreciate: months,
      });
      ...
      result.push({
        ...
        openingAccumulatedDepreciation: this.money(openingAccumulated),
        depreciationAmount: this.money(calculation.amount),
        closingAccumulatedDepreciation: this.money(
          calculation.closingAccumulatedDepreciation,
        ),
        openingNetBookValue: this.money(
          Number(asset.acquisitionCost) - openingAccumulated,
        ),
        closingNetBookValue: this.money(calculation.closingNetBookValue),
  ...
  private nextMonth(date: string): string {
