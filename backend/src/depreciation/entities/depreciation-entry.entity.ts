...
@Index('UQ_depreciation_entries_run_asset', ['runId', 'assetId'], {
  unique: true,
})
...
export class DepreciationEntryEntity {
  ...
  @Column({
    name: 'closing_accumulated_depreciation',
    type: 'numeric',
    precision: 18,
    scale: 2,
  })
  closingAccumulatedDepreciation!: string;
  @Column({
    name: 'opening_net_book_value',
    type: 'numeric',
    precision: 18,
    scale: 2,
  })
  ...
  @Column({
    name: 'closing_net_book_value',
    type: 'numeric',
    precision: 18,
    scale: 2,
  })
  closingNetBookValue!: string;
