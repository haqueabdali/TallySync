...
export class FinishedGoodsController {
  ...
  create(@Req() request: AuthenticatedRequest, @Body() dto: CreateFinishedGoodsReceiptDto) {
  ...
  findAll(@Req() request: AuthenticatedRequest, @Query() filter: FinishedGoodsReceiptFilterDto) {
    return this.service.findAll(request.user.companyId, filter);
  }

  @Get(':id')
  @ApiOkResponse({ description: 'Finished goods receipt details.' })
  findOne(@Req() request: AuthenticatedRequest, @Param('id', new ParseUUIDPipe()) id: string) {
