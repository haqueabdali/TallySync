...
@Index('IDX_inventory_cost_transactions_company_date', [
  'companyId',
  'transactionDate',
])
@Index('IDX_inventory_cost_transactions_item_warehouse', [
  'itemId',
  'warehouseId',
])
@Index(
  'UQ_inventory_cost_transactions_source_line',
  ['companyId', 'sourceType', 'sourceId', 'sourceLineId', 'transactionType'],
  { unique: true },
)
export class InventoryCostTransactionEntity {
  ...
  @Column({
    ...
    type: 'enum',
    enum: InventoryCostTransactionType,
    enumName: 'inventory_cost_transaction_type_enum',
  })
