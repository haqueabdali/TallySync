...
import type {
  InventoryCostExecutionOptions,
  RecordInventoryCostTransactionInput,
} from './interfaces/record-inventory-cost-transaction.interface';
...
export class InventoryCostEngineService {
  ...
  async getBalance(
    ...
    itemId: string,
    warehouseId: string,
  ): Promise<InventoryCostBalanceResponseDto> {
  ...
  async getTransactions(
    ...
    itemId: string,
    warehouseId: string,
  ): Promise<InventoryCostTransactionResponseDto[]> {
    ...
    return rows.map((row) => ({
      ...
      averageUnitCostAfter: row.averageUnitCostAfter,
  ...
  private async recordWithManager(
    ...
    const quantityAfter = this.round4(
      balance.quantity + direction * input.quantity,
    );
    if (quantityAfter < 0)
      throw new ConflictException(
        'Inventory cost transaction would create negative warehouse stock.',
      );
    const totalCost = this.round4(input.quantity * input.unitCost);
    const inventoryValueAfter = this.round4(
      balance.inventoryValue + direction * totalCost,
    );
    if (inventoryValueAfter < 0)
      throw new ConflictException(
        'Inventory cost transaction would create negative inventory value.',
      );
    const averageUnitCostAfter =
      quantityAfter === 0
        ? 0
        : this.round6(inventoryValueAfter / quantityAfter);
  ...
  private async assertReferences(
    ...
    const [item, warehouse] = await Promise.all([
      manager
        ...
        .findOne({ where: { id: input.itemId, companyId: input.companyId } }),
      manager
  ...
  private isInbound(type: InventoryCostTransactionType): boolean {
    return (
      type === InventoryCostTransactionType.RECEIPT ||
      type === InventoryCostTransactionType.ADJUSTMENT_IN ||
      type === InventoryCostTransactionType.LANDED_COST
    );
  }
