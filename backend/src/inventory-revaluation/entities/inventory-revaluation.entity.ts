...
@Index(
  'UQ_inventory_revaluations_company_number',
  ['companyId', 'revaluationNumber'],
  { unique: true },
)
@Index('IDX_inventory_revaluations_company_date', [
  'companyId',
  'revaluationDate',
])
export class InventoryRevaluationEntity {
  ...
  @Column({ name: 'revaluation_number', type: 'varchar', length: 50 })
  revaluationNumber!: string;
  @Column({ name: 'revaluation_date', type: 'date' }) revaluationDate!: string;
  @Column({
    type: 'enum',
    enum: InventoryRevaluationStatus,
    enumName: 'inventory_revaluation_status_enum',
    default: InventoryRevaluationStatus.DRAFT,
  })
  ...
  postedAt!: Date | null;
  @OneToMany(() => InventoryRevaluationLineEntity, (line) => line.revaluation, {
    cascade: ['insert'],
    eager: true,
  })
  lines!: InventoryRevaluationLineEntity[];
