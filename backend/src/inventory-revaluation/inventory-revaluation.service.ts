...
export class InventoryRevaluationService {
  ...
  async create(
    dto: CreateInventoryRevaluationDto,
    companyId: string,
    userId: string,
  ): Promise<InventoryRevaluationEntity> {
    if (
      new Set(dto.lines.map((line) => `${line.itemId}:${line.warehouseId}`))
        .size !== dto.lines.length
    )
      throw new BadRequestException(
        'Duplicate item and warehouse lines are not allowed.',
      );
      ...
      for (const input of dto.lines) {
        ...
        if (!balance)
          throw new NotFoundException(
            `Inventory balance not found for item ${input.itemId} and warehouse ${input.warehouseId}.`,
          );
        if (balance.quantity <= 0)
          throw new ConflictException(
            'Only positive on-hand quantities can be revalued.',
          );
        ...
        calculated.push(
          lineRepository.create({
            ...
            oldUnitCost: balance.averageUnitCost,
            newUnitCost: this.round6(input.newUnitCost),
            oldValue,
            newValue,
            adjustmentAmount: this.round4(newValue - oldValue),
          }),
        );
      }
      if (calculated.every((line) => line.adjustmentAmount === 0))
        throw new BadRequestException(
          'The revaluation does not change inventory value.',
        );
      const repo = manager.getRepository(InventoryRevaluationEntity);
      const entity = repo.create({
        companyId,
        revaluationNumber: await this.nextNumber(
          manager.getRepository(InventoryRevaluationEntity),
          companyId,
          dto.revaluationDate,
        ),
        revaluationDate: dto.revaluationDate,
        status: InventoryRevaluationStatus.DRAFT,
        gainAccountId: dto.gainAccountId,
        lossAccountId: dto.lossAccountId,
        currency: (dto.currency ?? 'EUR').toUpperCase(),
        totalIncrease: this.round4(
          calculated
            .filter((x) => x.adjustmentAmount > 0)
            .reduce((s, x) => s + x.adjustmentAmount, 0),
        ),
        totalDecrease: this.round4(
          calculated
            .filter((x) => x.adjustmentAmount < 0)
            .reduce((s, x) => s + Math.abs(x.adjustmentAmount), 0),
        ),
        journalEntryId: null,
  ...
  async post(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      const revaluation = await repo
        ...
        .getOne();
      if (!revaluation)
        throw new NotFoundException('Inventory revaluation not found.');
      if (revaluation.status !== InventoryRevaluationStatus.DRAFT)
        throw new ConflictException(
          'Only draft inventory revaluations can be posted.',
        );
      const settings = await manager
        .getRepository(AccountingSettingsEntity)
        .findOne({ where: { companyId } });
      if (!settings?.inventoryAccountId)
        throw new ConflictException('Inventory account is not configured.');
      ...
      for (const line of revaluation.lines) {
        const balance = await balanceRepo
          ...
          .setLock('pessimistic_write')
          .where(
            'b.companyId = :companyId AND b.itemId = :itemId AND b.warehouseId = :warehouseId',
            { companyId, itemId: line.itemId, warehouseId: line.warehouseId },
          )
          .getOne();
        if (!balance)
          throw new NotFoundException('Inventory balance no longer exists.');
        if (
          this.round4(balance.quantity) !== this.round4(line.quantity) ||
          this.round4(balance.inventoryValue) !== this.round4(line.oldValue)
        )
          throw new ConflictException(
            'Inventory changed after the revaluation was prepared. Create a new revaluation.',
          );
        balance.averageUnitCost = line.newUnitCost;
        balance.inventoryValue = line.newValue;
        ...
        await transactionRepo.save(
          transactionRepo.create({
            ...
            totalCost: Math.abs(line.adjustmentAmount),
            quantityAfter: balance.quantity,
            averageUnitCostAfter: balance.averageUnitCost,
            inventoryValueAfter: balance.inventoryValue,
            createdBy: userId,
          }),
        );
      }
      ...
      revaluation.status = InventoryRevaluationStatus.POSTED;
      revaluation.journalEntryId = journal.id;
      revaluation.postedBy = userId;
      revaluation.postedAt = new Date();
      return repo.save(revaluation);
    });
  }
  async findAll(
    filter: InventoryRevaluationFilterDto,
    companyId: string,
  ): Promise<{
    data: InventoryRevaluationEntity[];
    total: number;
    page: number;
    limit: number;
  }> {
    ...
    if (filter.dateFrom)
      qb.andWhere('r.revaluationDate >= :dateFrom', {
        dateFrom: filter.dateFrom,
      });
    if (filter.dateTo)
      qb.andWhere('r.revaluationDate <= :dateTo', { dateTo: filter.dateTo });
    const [data, total] = await qb
      .orderBy('r.revaluationDate', 'DESC')
  ...
  async findOne(
    ...
    if (!row) throw new NotFoundException('Inventory revaluation not found.');
    return row;
  }
  private async validateAccounts(
    ...
    const [gain, loss] = await Promise.all([
      repo.findOne({ where: { id: gainId, companyId } }),
      repo.findOne({ where: { id: lossId, companyId } }),
    ]);
    if (
      !gain ||
      gain.status !== AccountStatus.ACTIVE ||
      gain.isGroup ||
      gain.type !== AccountType.INCOME
    )
      throw new BadRequestException(
        'Gain account must be an active posting income account.',
      );
    if (
      !loss ||
      loss.status !== AccountStatus.ACTIVE ||
      loss.isGroup ||
      loss.type !== AccountType.EXPENSE
    )
      throw new BadRequestException(
        'Loss account must be an active posting expense account.',
      );
  }
  private async createJournal(
    manager: import('typeorm').EntityManager,
    r: InventoryRevaluationEntity,
    ...
    if (r.totalIncrease > 0) {
      lines.push(
        {
          accountId: inventoryAccountId,
          debit: r.totalIncrease,
          credit: 0,
          description: `Inventory revaluation ${r.revaluationNumber}`,
        },
        {
          accountId: r.gainAccountId,
          debit: 0,
          credit: r.totalIncrease,
          description: `Inventory revaluation gain ${r.revaluationNumber}`,
        },
      );
    }
    if (r.totalDecrease > 0) {
      lines.push(
        {
          accountId: r.lossAccountId,
          debit: r.totalDecrease,
          credit: 0,
          description: `Inventory revaluation loss ${r.revaluationNumber}`,
        },
        {
          accountId: inventoryAccountId,
          debit: 0,
          credit: r.totalDecrease,
          description: `Inventory revaluation ${r.revaluationNumber}`,
        },
      );
    }
    const total = this.round2(r.totalIncrease + r.totalDecrease);
    const entry = await entryRepo.save(
      entryRepo.create({
        companyId: r.companyId,
        entryNumber: await this.nextJournalNumber(
          entryRepo,
          r.companyId,
          r.revaluationDate,
        ),
        entryDate: r.revaluationDate,
        status: JournalEntryStatus.POSTED,
        sourceType: JournalEntrySourceType.INVENTORY_ADJUSTMENT,
        sourceId: r.id,
        referenceNumber: r.revaluationNumber,
        currency: r.currency,
        totalDebit: total,
        totalCredit: total,
        narration: `Inventory revaluation ${r.revaluationNumber}`,
        createdBy: userId,
        updatedBy: userId,
        postedBy: userId,
        postedAt: new Date(),
  ...
  private async nextNumber(
    repo: Repository<InventoryRevaluationEntity>,
    companyId: string,
    date: string,
  ): Promise<string> {
    ...
    const count = await repo
      .createQueryBuilder('r')
      .where('r.companyId = :companyId', { companyId })
      .andWhere('r.revaluationNumber LIKE :prefix', { prefix: `IR-${year}-%` })
      .getCount();
    return `IR-${year}-${String(count + 1).padStart(6, '0')}`;
  }
  private async nextJournalNumber(
    repo: Repository<JournalEntryEntity>,
    companyId: string,
    date: string,
  ): Promise<string> {
    const year = date.slice(0, 4);
    const count = await repo
      ...
      .andWhere('e.entryNumber LIKE :prefix', { prefix: `JE-${year}-%` })
      .getCount();
