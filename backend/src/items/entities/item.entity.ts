...
export class ItemEntity {
  ...
  @Column({ type: 'varchar', length: 100, nullable: true }) barcode!:
    string | null;
  ...
  @Column({
    ...
    default: ItemSyncStatus.PENDING,
  })
  syncStatus!: ItemSyncStatus;
  @Column({ name: 'sync_error', type: 'text', nullable: true }) syncError!:
    string | null;
  ...
  get isLowStock(): boolean {
    return (
      this.trackInventory &&
      Number(this.currentStock) <= Number(this.minimumStock)
    );
  }
