...
@Index(
  'UQ_manual_cost_adjustments_company_number',
  ['companyId', 'adjustmentNumber'],
  { unique: true },
)
@Index('IDX_manual_cost_adjustments_company_date', [
  'companyId',
  'adjustmentDate',
])
export class ManualCostAdjustmentEntity {
  ...
  @Column({ name: 'adjustment_number', type: 'varchar', length: 40 })
  adjustmentNumber!: string;
  @Column({ name: 'adjustment_date', type: 'date' }) adjustmentDate!: string;
  @Column({
    type: 'enum',
    enum: ManualCostAdjustmentStatus,
    enumName: 'manual_cost_adjustment_status_enum',
    default: ManualCostAdjustmentStatus.DRAFT,
  })
  status!: ManualCostAdjustmentStatus;
  ...
  @Column({ name: 'posted_by', type: 'uuid', nullable: true }) postedBy!:
    string | null;
  ...
  updatedAt!: Date;
  @OneToMany(() => ManualCostAdjustmentLineEntity, (line) => line.adjustment, {
    cascade: true,
  })
  lines!: ManualCostAdjustmentLineEntity[];
}
