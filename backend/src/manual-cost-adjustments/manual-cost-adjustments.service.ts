...
import type {
  CreateManualCostAdjustmentDto,
  CreateManualCostAdjustmentLineDto,
} from './dto/create-manual-cost-adjustment.dto';
...
export class ManualCostAdjustmentsService {
  ...
  async create(
    dto: CreateManualCostAdjustmentDto,
    companyId: string,
    userId: string,
  ): Promise<ManualCostAdjustmentEntity> {
    const uniqueKeys = new Set(
      dto.lines.map((line) => `${line.itemId}:${line.warehouseId}`),
    );
    if (uniqueKeys.size !== dto.lines.length) {
      throw new BadRequestException(
        'Only one adjustment line is allowed per item and warehouse.',
      );
    }
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      for (const input of dto.lines) {
        calculatedLines.push(
          await this.calculateLine(manager, input, companyId, lineRepository),
        );
      }

      const totalIncrease = this.round4(
        calculatedLines
          .filter((line) => line.valueChange > 0)
          .reduce((sum, line) => sum + line.valueChange, 0),
      );
      const totalDecrease = this.round4(
        calculatedLines
          .filter((line) => line.valueChange < 0)
          .reduce((sum, line) => sum + Math.abs(line.valueChange), 0),
      );
      if (totalIncrease === 0 && totalDecrease === 0)
        throw new BadRequestException(
          'The adjustment does not change inventory quantity or value.',
        );
      ...
      return repository.save(
        repository.create({
          companyId,
          adjustmentNumber: await this.nextNumber(
            repository,
            companyId,
            dto.adjustmentDate,
          ),
  ...
  async post(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (!adjustment)
        throw new NotFoundException('Manual cost adjustment not found.');
      if (adjustment.status !== ManualCostAdjustmentStatus.DRAFT)
        throw new ConflictException(
          'Only draft manual cost adjustments can be posted.',
        );
      ...
      for (const line of adjustment.lines) {
        const balance = await balanceRepository
          .createQueryBuilder('balance')
          .setLock('pessimistic_write')
          .where(
            'balance.companyId = :companyId AND balance.itemId = :itemId AND balance.warehouseId = :warehouseId',
            {
              companyId,
              itemId: line.itemId,
        ...
        if (
          ...
          throw new ConflictException(
            'Inventory changed after the adjustment was prepared. Create a new adjustment.',
          );
        }
  ...
  async findAll(
    filter: ManualCostAdjustmentFilterDto,
    companyId: string,
  ): Promise<{
    data: ManualCostAdjustmentEntity[];
    total: number;
    page: number;
    limit: number;
  }> {
    ...
    if (filter.status)
      query.andWhere('adjustment.status = :status', { status: filter.status });
    if (filter.dateFrom)
      query.andWhere('adjustment.adjustmentDate >= :dateFrom', {
        dateFrom: filter.dateFrom,
      });
    if (filter.dateTo)
      query.andWhere('adjustment.adjustmentDate <= :dateTo', {
  ...
  private async calculateLine(
    ...
    if (!warehouse)
      throw new NotFoundException(
        `Active warehouse ${input.warehouseId} was not found.`,
      );
    ...
    switch (input.adjustmentType) {
      case ManualCostAdjustmentType.QUANTITY_IN:
        if (input.quantity === undefined || input.unitCost === undefined)
          throw new BadRequestException(
            'Quantity and unit cost are required for a quantity increase.',
          );
      ...
      case ManualCostAdjustmentType.QUANTITY_OUT:
        if (input.quantity === undefined)
          throw new BadRequestException(
            'Quantity is required for a quantity decrease.',
          );
        quantityChange = -this.round4(input.quantity);
        if (Math.abs(quantityChange) > oldQuantity)
          throw new ConflictException(
            'Quantity decrease cannot create negative inventory.',
          );
      ...
      case ManualCostAdjustmentType.VALUE_IN:
        if (input.valueAmount === undefined)
          throw new BadRequestException(
            'Value amount is required for a value increase.',
          );
        if (oldQuantity <= 0)
          throw new ConflictException(
            'Value-only adjustments require positive on-hand quantity.',
          );
      ...
      case ManualCostAdjustmentType.VALUE_OUT:
        if (input.valueAmount === undefined)
          throw new BadRequestException(
            'Value amount is required for a value decrease.',
          );
        if (oldQuantity <= 0)
          throw new ConflictException(
            'Value-only adjustments require positive on-hand quantity.',
        ...
        valueChange = -this.round4(input.valueAmount);
        if (Math.abs(valueChange) > oldValue)
          throw new ConflictException(
            'Value decrease cannot create negative inventory value.',
          );
        break;
    }
    ...
    if (newQuantity < 0 || newValue < 0)
      throw new ConflictException(
        'Adjustment cannot create negative inventory quantity or value.',
      );
    const newAverage =
      newQuantity === 0 ? 0 : this.round6(newValue / newQuantity);
  ...
  private async createJournal(
    manager: EntityManager,
    adjustment: ManualCostAdjustmentEntity,
      ...
      lines.push(
        {
          ...
          debit: adjustment.totalIncrease,
          credit: 0,
          description: `Manual cost adjustment ${adjustment.adjustmentNumber}`,
        },
        {
          accountId: adjustment.gainAccountId,
          debit: 0,
          credit: adjustment.totalIncrease,
          description: `Inventory adjustment gain ${adjustment.adjustmentNumber}`,
        },
      );
    }
    if (adjustment.totalDecrease > 0) {
      lines.push(
        {
          accountId: adjustment.lossAccountId,
          debit: adjustment.totalDecrease,
          credit: 0,
          description: `Inventory adjustment loss ${adjustment.adjustmentNumber}`,
        },
        {
          accountId: inventoryAccountId,
          debit: 0,
          credit: adjustment.totalDecrease,
          description: `Manual cost adjustment ${adjustment.adjustmentNumber}`,
        },
      );
    }
    const total = this.round2(
      adjustment.totalIncrease + adjustment.totalDecrease,
    );
    ...
    entry.lines = await lineRepository.save(
      lines.map((line) =>
  ...
  private async nextNumber(
    repository: Repository<ManualCostAdjustmentEntity>,
    companyId: string,
    date: string,
  ): Promise<string> {
    ...
    const count = await repository
      .createQueryBuilder('adjustment')
      .where('adjustment.companyId = :companyId', { companyId })
      .andWhere('adjustment.adjustmentNumber LIKE :prefix', {
        prefix: `MCA-${year}-%`,
      })
      .getCount();
  ...
  private async nextJournalNumber(
    repository: Repository<JournalEntryEntity>,
    companyId: string,
    date: string,
  ): Promise<string> {
    const year = date.slice(0, 4);
    const count = await repository
      ...
      .andWhere('entry.entryNumber LIKE :prefix', { prefix: `JE-${year}-%` })
      .getCount();
