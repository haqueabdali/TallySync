...
@RequireLicenseFeature(LicensedFeature.WIP)
...
export class ManufacturingWipAccountingController {
  ...
  upsertSettings(
    ...
    @Body() dto: UpsertWipAccountingSettingsDto,
  ) {
  ...
  @ApiOperation({
    summary: 'Transfer posted material consumption cost into WIP',
  })
  @ApiCreatedResponse()
  postConsumption(
    @Req() request: AuthenticatedRequest,
    ...
    return this.service.postMaterialConsumption(
      request.user.companyId,
      request.user.id,
      id,
    );
  ...
  @ApiOperation({
    summary: 'Transfer posted finished-goods cost from WIP into inventory',
  })
  @ApiCreatedResponse()
  postReceipt(
    @Req() request: AuthenticatedRequest,
    ...
    return this.service.postFinishedGoodsReceipt(
      request.user.companyId,
      request.user.id,
      id,
    );
  ...
  list(
    @Req() request: AuthenticatedRequest,
    @Query() filter: WipPostingFilterDto,
  ) {
  ...
  summary(
    ...
    return this.service.getProductionOrderSummary(request.user.companyId, id);
  }
}
