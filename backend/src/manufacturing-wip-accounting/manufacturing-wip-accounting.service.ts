...
export class ManufacturingWipAccountingService {
  ...
  async getSettings(companyId: string): Promise<WipAccountingSettingsEntity> {
    const settings = await this.settingsRepository.findOne({
      where: { companyId },
      relations: { wipAccount: true },
    });
    if (!settings)
      throw new NotFoundException(
        'Manufacturing WIP accounting settings are not configured.',
      );
    return settings;
  }
  ...
  async upsertSettings(
    companyId: string,
    userId: string,
    dto: UpsertWipAccountingSettingsDto,
  ): Promise<WipAccountingSettingsEntity> {
    const account = await this.accountRepository.findOne({
      where: { id: dto.wipAccountId, companyId },
    });
    ...
    if (
      ...
      throw new BadRequestException(
        'WIP account must be an active posting ASSET account.',
      );
    }
  ...
  async postMaterialConsumption(
  ...
  ): Promise<WipPostingEntity> {
    const existing = await this.postingRepository.findOne({
      where: {
        companyId,
        postingType: WipPostingType.MATERIAL_CONSUMPTION,
    ...
    const consumption = await this.consumptionRepository.findOne({
      ...
      relations: { lines: true, productionOrder: true },
    });
    if (!consumption)
      throw new NotFoundException('Material consumption not found.');
    if (consumption.status !== MaterialConsumptionStatus.POSTED)
      throw new BadRequestException(
        'Only posted material consumption can be transferred to WIP.',
      );
    const amount = this.money(
      consumption.lines.reduce((sum, line) => sum + Number(line.totalCost), 0),
    );
    if (amount <= 0)
      throw new BadRequestException(
        'Material consumption has no positive cost to post.',
      );
    const { inventoryAccountId, wipAccountId } =
      await this.resolveAccounts(companyId);
        ...
        lines: [
          {
            ...
            debit: amount,
            credit: 0,
            description: `WIP - ${consumption.productionOrder.orderNumber}`,
          },
          {
            accountId: inventoryAccountId,
            debit: 0,
            credit: amount,
            description: `Materials issued - ${consumption.consumptionNumber}`,
          },
        ],
      },
      companyId,
    ...
    try {
      return await this.postingRepository.save(
        this.postingRepository.create({
          companyId,
          productionOrderId: consumption.productionOrderId,
          postingType: WipPostingType.MATERIAL_CONSUMPTION,
          sourceId: consumption.id,
          postingDate: consumption.consumptionDate,
          amount,
      ...
      if (this.isUniqueViolation(error))
        throw new ConflictException(
          'Material consumption has already been posted to WIP.',
        );
      throw error;
    }
  }

  async postFinishedGoodsReceipt(
    companyId: string,
    userId: string,
    receiptId: string,
  ): Promise<WipPostingEntity> {
    const existing = await this.postingRepository.findOne({
      where: {
        companyId,
        postingType: WipPostingType.FINISHED_GOODS_RECEIPT,
        sourceId: receiptId,
      },
    });
    if (existing) return existing;
    const receipt = await this.receiptRepository.findOne({
      ...
      relations: { productionOrder: true },
    });
    if (!receipt)
      throw new NotFoundException('Finished-goods receipt not found.');
    if (receipt.status !== FinishedGoodsReceiptStatus.POSTED)
      throw new BadRequestException(
        'Only posted finished-goods receipts can be transferred from WIP.',
      );
    const amount = this.money(Number(receipt.totalCost));
    if (amount <= 0)
      throw new BadRequestException(
        'Finished-goods receipt has no positive cost to post.',
      );
    const { inventoryAccountId, wipAccountId } =
      await this.resolveAccounts(companyId);
    const availableWip = await this.getAvailableWip(
      companyId,
      receipt.productionOrderId,
    );
    if (amount > availableWip + 0.01)
      throw new BadRequestException(
        `Receipt cost exceeds available WIP by ${(amount - availableWip).toFixed(2)}.`,
      );
    const journal = await this.journalEntriesService.create(
      {
        ...
        lines: [
          ...
          },
          {
            ...
            debit: 0,
            credit: amount,
            description: `WIP released - ${receipt.productionOrder.orderNumber}`,
          },
        ],
      },
      companyId,
    ...
    try {
      ...
      if (this.isUniqueViolation(error))
        throw new ConflictException(
          'Finished-goods receipt has already been posted from WIP.',
        );
      throw error;
    }
  }
    ...
    const [data, total] = await this.postingRepository.findAndCount({
      where: {
        ...
        ...(filter.productionOrderId
          ...
          : {}),
        ...(filter.postingType ? { postingType: filter.postingType } : {}),
      },
  ...
  async getProductionOrderSummary(
    companyId: string,
    ...
    const raw = await this.postingRepository
      .createQueryBuilder('posting')
      .select(
        'COALESCE(SUM(CASE WHEN posting.posting_type = :consumption THEN posting.amount ELSE 0 END), 0)',
        'materialCost',
      )
      .addSelect(
        'COALESCE(SUM(CASE WHEN posting.posting_type = :receipt THEN posting.amount ELSE 0 END), 0)',
        'finishedGoodsCost',
      )
      ...
      .setParameters({
        consumption: WipPostingType.MATERIAL_CONSUMPTION,
        receipt: WipPostingType.FINISHED_GOODS_RECEIPT,
      })
    ...
    return {
      ...
      materialCost,
      finishedGoodsCost,
      openWip: this.money(materialCost - finishedGoodsCost),
    };
  }

  private async resolveAccounts(
    companyId: string,
  ): Promise<{ inventoryAccountId: string; wipAccountId: string }> {
    const accounting = await this.accountingSettingsRepository.findOne({
      where: { companyId },
    });
    if (!accounting?.inventoryAccountId)
      throw new BadRequestException(
        'Inventory account is not configured in accounting settings.',
      );
    const wip = await this.getSettings(companyId);
    return {
      inventoryAccountId: accounting.inventoryAccountId,
      wipAccountId: wip.wipAccountId,
    };
  }

  private async getAvailableWip(
    companyId: string,
    productionOrderId: string,
  ): Promise<number> {
    const raw = await this.postingRepository
      .createQueryBuilder('posting')
      .select(
        'COALESCE(SUM(CASE WHEN posting.posting_type = :consumption THEN posting.amount ELSE -posting.amount END), 0)',
        'amount',
      )
      .where('posting.company_id = :companyId', { companyId })
      .andWhere('posting.production_order_id = :productionOrderId', {
