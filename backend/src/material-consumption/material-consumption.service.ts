...
export class MaterialConsumptionService {
  ...
  async findAll(
    companyId: string,
    filter: MaterialConsumptionFilterDto,
  ): Promise<{
    ...
    if (filter.productionOrderId)
      query.andWhere('consumption.productionOrderId = :productionOrderId', {
        productionOrderId: filter.productionOrderId,
      });
    if (filter.status)
      query.andWhere('consumption.status = :status', { status: filter.status });
    if (filter.dateFrom)
      query.andWhere('consumption.consumptionDate >= :dateFrom', {
        dateFrom: filter.dateFrom,
      });
    if (filter.dateTo)
      query.andWhere('consumption.consumptionDate <= :dateTo', {
  ...
  private async createWithManager(
    ...
    const duplicate = await manager
      .getRepository(MaterialConsumptionEntity)
      .findOne({
        where: { companyId, consumptionNumber: dto.consumptionNumber },
      });
    if (duplicate)
      throw new ConflictException(
        'Material consumption number already exists.',
      );
    ...
    if (
      ...
      throw new BadRequestException(
        'Material can only be consumed for a released or in-progress production order.',
      );
    }

    const componentById = new Map(
      productionOrder.components.map((component) => [component.id, component]),
    );
    const consumptionRepository = manager.getRepository(
    ...
    for (const inputLine of dto.lines) {
      ...
      if (!component)
        throw new BadRequestException(
          `Production order component ${inputLine.productionOrderComponentId} is invalid.`,
        );

      const remainingRequired = this.round6(
        component.requiredQuantity - component.consumedQuantity,
      );
      if (inputLine.quantity > remainingRequired) {
        throw new BadRequestException(
          `Consumption quantity exceeds the remaining requirement for component ${component.componentItemId}.`,
        );
      }

      const costTransaction =
        await this.movingAverageCostingService.recordIssue(
          {
            companyId,
      ...
      component.consumedQuantity = this.round6(
        component.consumedQuantity + inputLine.quantity,
      );
      await manager
        .getRepository(ProductionOrderComponentEntity)
        .save(component);
