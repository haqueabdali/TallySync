...
export class NegativeInventoryPolicyController {
  ...
  create(
    ...
    @Body() dto: CreateNegativeInventoryPolicyDto,
  ) {
  ...
  findAll(
    @Req() request: AuthenticatedRequest,
    @Query() filter: NegativeInventoryPolicyFilterDto,
  ) {
  ...
  @Post('evaluate')
  @ApiOperation({
    summary: 'Evaluate a proposed stock issue against the resolved policy',
  })
  evaluate(
    @Req() request: AuthenticatedRequest,
    @Body() dto: EvaluateNegativeInventoryDto,
  ) {
  ...
  update(
    @Req() request: AuthenticatedRequest,
    @Param('id', ParseUUIDPipe) id: string,
    @Body() dto: UpdateNegativeInventoryPolicyDto,
  ) {
