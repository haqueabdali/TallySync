...
export class NegativeInventoryPolicyService {
  ...
  ): Promise<NegativeInventoryPolicyEntity> {
  ...
  async findAll(
    companyId: string,
    filter: NegativeInventoryPolicyFilterDto,
  ): Promise<NegativeInventoryPolicyEntity[]> {
    ...
    if (filter.warehouseId)
      query.andWhere('policy.warehouseId = :warehouseId', {
        warehouseId: filter.warehouseId,
      });
    if (filter.itemId)
      query.andWhere('policy.itemId = :itemId', { itemId: filter.itemId });
    if (filter.isActive !== undefined)
      query.andWhere('policy.isActive = :isActive', {
  ...
  async findOne(
    ...
    if (!policy)
      throw new NotFoundException('Negative inventory policy not found.');
    return policy;
  }
  ...
  async update(
    companyId: string,
    id: string,
    dto: UpdateNegativeInventoryPolicyDto,
  ): Promise<NegativeInventoryPolicyEntity> {
    const policy = await this.findOne(companyId, id);
    const warehouseId =
      dto.warehouseId !== undefined
        ? dto.warehouseId
        : (policy.warehouseId ?? undefined);
    const itemId =
      dto.itemId !== undefined ? dto.itemId : (policy.itemId ?? undefined);
    const mode = dto.mode ?? policy.mode;
    const maxNegativeQuantity =
      dto.maxNegativeQuantity !== undefined
        ? dto.maxNegativeQuantity
        : policy.maxNegativeQuantity === null
          ? undefined
    ...
    policy.mode = mode;
    policy.maxNegativeQuantity =
      mode === NegativeInventoryMode.ALLOW_WITH_LIMIT
        ? String(maxNegativeQuantity)
        : null;
  ...
  async resolve(
    ...
    warehouseId: string,
    itemId: string,
  ): Promise<NegativeInventoryPolicyEntity | null> {
  ...
  async evaluate(
    input: NegativeInventoryCheckInput,
  ): Promise<NegativeInventoryDecision> {
    if (input.issueQuantity <= 0)
      throw new BadRequestException(
        'Issue quantity must be greater than zero.',
      );
    const projectedQuantity = this.round(
      input.currentQuantity - input.issueQuantity,
    );
    ...
    const maxNegativeQuantity =
      policy?.maxNegativeQuantity === null ||
      policy?.maxNegativeQuantity === undefined
        ? null
        : Number(policy.maxNegativeQuantity);
    ...
    if (
      mode === NegativeInventoryMode.ALLOW_WITH_LIMIT &&
      maxNegativeQuantity !== null &&
      projectedQuantity >= -maxNegativeQuantity
    ) {
      return {
        allowed: true,
        projectedQuantity,
  ...
  private validateScope(warehouseId?: string, itemId?: string): void {
    if (itemId && !warehouseId)
      throw new BadRequestException('itemId requires warehouseId.');
  }
  ...
  private validateLimit(
    ...
    if (
      mode === NegativeInventoryMode.ALLOW_WITH_LIMIT &&
      maxNegativeQuantity === undefined
    ) {
      throw new BadRequestException(
        'maxNegativeQuantity is required for allow_with_limit.',
      );
    }
  }
  ...
  private async validateReferences(
    ...
    if (warehouseId) {
      ...
      if (!warehouse)
        throw new BadRequestException(
          'Warehouse does not belong to the company.',
        );
    }
    if (itemId) {
      ...
      if (!item)
        throw new BadRequestException('Item does not belong to the company.');
    }
  }

  private async ensureUniqueScope(
    companyId: string,
    ...
    const query = this.policyRepository
      ...
      .andWhere(
        warehouseId === null
          ? 'policy.warehouseId IS NULL'
          : 'policy.warehouseId = :warehouseId',
        { warehouseId },
      )
      .andWhere(
        itemId === null ? 'policy.itemId IS NULL' : 'policy.itemId = :itemId',
        { itemId },
      );
    if (excludeId) query.andWhere('policy.id <> :excludeId', { excludeId });
    if (await query.getOne())
      throw new ConflictException('A policy already exists for this scope.');
  }
