...
export class NotificationsService {
  ...
  registerAdapter(
    ...
    if (this.adapters.has(channel)) {
      throw new ConflictException(
        `An adapter is already registered for ${channel}`,
      );
    }
  ...
  async createTemplate(
    ...
    try {
      ...
      if (this.isUniqueViolation(error)) {
        throw new ConflictException(
          'A notification template already exists for this code and channel',
        );
      }
      throw error;
    }
  ...
  async updateTemplate(
    ...
    if (dto.subjectTemplate !== undefined)
      template.subjectTemplate = dto.subjectTemplate.trim() || null;
  ...
  async create(
    ...
    if (
      ...
      throw new ConflictException(
        `The recipient has disabled ${dto.channel} notifications`,
      );
    }
  ...
  async sendFromTemplate(
    ...
    return this.create(companyId, userId, {
      ...
      subject: template.subjectTemplate
        ? this.render(template.subjectTemplate, dto.variables)
        : undefined,
  ...
  async findAll(
    ...
    if (query.recipientUserId)
      qb.andWhere('notification.recipientUserId = :recipientUserId', {
  ...
  async cancel(companyId: string, id: string): Promise<NotificationEntity> {
    ...
    if (
      ...
      throw new ConflictException(
        `A ${notification.status} notification cannot be cancelled`,
      );
    }
  ...
  async upsertPreference(
    ...
    let preference = await this.preferenceRepository.findOne({
      where: { companyId, userId, channel: dto.channel },
    });
    if (!preference) {
  ...
  async deliver(
    ...
    try {
      ...
      if (!adapter) {
        throw new Error(
          `No notification adapter is registered for ${notification.channel}`,
        );
      }
      ...
      notification.errorMessage = this.getErrorMessage(error).substring(
        0,
        10000,
      );
      await this.notificationRepository.save(notification);
  ...
  private async queue(
    ...
    const job = await this.backgroundJobsService.enqueue(
  ...
  private async isChannelEnabled(
    ...
    const preference = await this.preferenceRepository.findOne({
      where: { companyId, userId, channel },
    });
  ...
  private validateRecipient(
    ...
    if (channel === NotificationChannel.IN_APP && !recipientUserId) {
      throw new BadRequestException(
        'recipientUserId is required for in-app notifications',
      );
    }
  }
