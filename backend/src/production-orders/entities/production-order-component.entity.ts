...
@Index(
  'uq_production_order_component_item',
  ['productionOrderId', 'componentItemId'],
  { unique: true },
)
@Check(
  'chk_production_order_component_required_positive',
  '"required_quantity" > 0',
)
@Check(
  'chk_production_order_component_consumed_non_negative',
  '"consumed_quantity" >= 0',
)
export class ProductionOrderComponentEntity {
  ...
  @ManyToOne(() => ProductionOrderEntity, (order) => order.components, {
    nullable: false,
    onDelete: 'CASCADE',
  })
