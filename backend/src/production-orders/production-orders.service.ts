...
export class ProductionOrdersService {
  ...
  async findAll(
    ...
    if (filter.warehouseId) {
      query.andWhere('order.warehouseId = :warehouseId', {
        warehouseId: filter.warehouseId,
      });
    }

    if (filter.finishedItemId) {
      query.andWhere('order.finishedItemId = :finishedItemId', {
  ...
  async update(
    ...
    if (current.status !== ProductionOrderStatus.DRAFT) {
    ...
    const [bom, warehouse] = await Promise.all([
      this.getActiveBom(bomId, companyId),
      this.getWarehouse(warehouseId, companyId),
    ]);
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      await manager.delete(ProductionOrderComponentEntity, {
        productionOrderId: current.id,
      });

      await manager.save(
        bom.components.map((component) =>
          manager.create(ProductionOrderComponentEntity, {
            productionOrderId: current.id,
            componentItemId: component.componentItemId,
            bomQuantity: component.quantity,
            scrapPercentage: component.scrapPercentage,
            requiredQuantity: this.roundQuantity(
              component.quantity *
                multiplier *
                (1 + component.scrapPercentage / 100),
            ),
  ...
  async complete(
     ...
     */
    const existing = await this.getEntity(id, companyId);
    ...
    if (existing.status === ProductionOrderStatus.COMPLETED) {
       ...
       */
      await this.autoPostProductionCompletionIfEnabled(id, companyId, userId);
    ...
    await this.dataSource.transaction(async (manager) => {
      const repository = manager.getRepository(ProductionOrderEntity);
      ...
      const completedQuantity = Number(order.completedQuantity ?? 0);
       ...
       */
      order.completedQuantity = this.roundQuantity(
        Math.max(completedQuantity, plannedQuantity),
      );
       ...
       */
      const actualMaterialCost = Number(order.actualMaterialCost ?? 0);

      const actualLaborCost = Number(order.actualLaborCost ?? 0);

      const actualOverheadCost = Number(order.actualOverheadCost ?? 0);

      order.actualTotalCost = this.roundMoney(
  ...
  async cancel(
    ...
    if (
      ![ProductionOrderStatus.DRAFT, ProductionOrderStatus.RELEASED].includes(
        order.status,
      )
    ) {
      throw new BadRequestException(
  ...
  private async autoPostProductionCompletionIfEnabled(
    ...
    if (settings?.autoPostProductionCompletion === false) {
      return;
    }
  ...
  private async getActiveBom(
    ...
    if (!bom) {
      throw new BadRequestException('Active bill of material not found');
    }

    if (bom.components.length === 0) {
      throw new BadRequestException('Bill of material has no components');
    }
