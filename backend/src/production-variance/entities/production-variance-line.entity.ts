...
@Index(
  'uq_production_variance_line_component',
  ['productionVarianceId', 'productionOrderComponentId'],
  { unique: true },
)
export class ProductionVarianceLineEntity {
  @PrimaryGeneratedColumn('uuid') id!: string;
  @Column({ name: 'production_variance_id', type: 'uuid' })
  productionVarianceId!: string;
  ...
  actualCost!: number;
  @ManyToOne(() => ProductionVarianceEntity, (variance) => variance.lines, {
    nullable: false,
    onDelete: 'CASCADE',
  })
  @JoinColumn({ name: 'production_variance_id' })
  productionVariance!: ProductionVarianceEntity;
