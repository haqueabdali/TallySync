...
@Index('uq_production_variances_order', ['companyId', 'productionOrderId'], {
  unique: true,
})
@Check(
  'chk_production_variance_costs_non_negative',
  '"material_cost" >= 0 AND "finished_goods_cost" >= 0',
)
export class ProductionVarianceEntity {
  ...
  @Column({
    type: 'enum',
    enum: ProductionVarianceStatus,
    enumName: 'production_variance_status_enum',
    default: ProductionVarianceStatus.CALCULATED,
  })
  ...
  @OneToMany(
    ...
    (line) => line.productionVariance,
    { cascade: ['insert'] },
  )
  lines!: ProductionVarianceLineEntity[];
}
