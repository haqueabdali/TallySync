...
export class ProductionVarianceController {
  ...
  @Get('settings') @ApiOkResponse()
  getSettings(@Req() request: AuthenticatedRequest) { return this.service.getSettings(request.user.companyId); }

  @Put('settings') @ApiOkResponse()
  upsertSettings(@Req() request: AuthenticatedRequest, @Body() dto: UpsertProductionVarianceSettingsDto) {
    return this.service.upsertSettings(request.user.companyId, request.user.id, dto);
  }

  @Post('production-orders/:productionOrderId/calculate')
  @ApiOperation({ summary: 'Calculate usage and WIP variance for a completed production order' })
  @ApiCreatedResponse()
  calculate(
    @Req() request: AuthenticatedRequest,
    @Param('productionOrderId', ParseUUIDPipe) productionOrderId: string,
    @Body() dto: CalculateProductionVarianceDto,
  ) { return this.service.calculate(request.user.companyId, request.user.id, productionOrderId, dto); }

  @Post(':id/post') @ApiOkResponse()
  post(@Req() request: AuthenticatedRequest, @Param('id', ParseUUIDPipe) id: string) {
    return this.service.post(request.user.companyId, request.user.id, id);
  }

  @Get() @ApiOkResponse()
  list(@Req() request: AuthenticatedRequest, @Query() filter: ProductionVarianceFilterDto) {
