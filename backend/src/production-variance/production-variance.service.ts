...
export class ProductionVarianceService {
  constructor(
    ...
    private readonly settingsRepository: Repository<ProductionVarianceSettingsEntity>,

    @InjectRepository(ProductionVarianceEntity)
    private readonly varianceRepository: Repository<ProductionVarianceEntity>,

    @InjectRepository(ProductionVarianceLineEntity)
    private readonly lineRepository: Repository<ProductionVarianceLineEntity>,
    ...
    private readonly consumptionLineRepository: Repository<MaterialConsumptionLineEntity>,

    @InjectRepository(WipPostingEntity)
    private readonly wipPostingRepository: Repository<WipPostingEntity>,
  ...
  async upsertSettings(
    ...
    if (dto.favorableVarianceAccountId === dto.unfavorableVarianceAccountId) {
  ...
  async calculate(
    ...
    const existing = await this.varianceRepository.findOne({
      where: {
        companyId,
        productionOrderId,
      },
    ...
    const costs = await this.getWipCosts(companyId, productionOrderId);

    const actualCosts = await this.getComponentActualCosts(productionOrderId);

    const totalVariance = this.money(
      costs.materialCost - costs.finishedGoodsCost,
    );
    ...
    const lines = order.components.map((component) =>
      this.lineRepository.create({
        ...
        requiredQuantity: Number(component.requiredQuantity),
        consumedQuantity: Number(component.consumedQuantity),
        quantityVariance: this.quantity(
          Number(component.consumedQuantity) -
            Number(component.requiredQuantity),
        ),
        actualCost: this.money(actualCosts.get(component.id) ?? 0),
      }),
    );
    ...
    const variance = this.varianceRepository.create({
      companyId,
      productionOrderId,
      varianceDate: dto.varianceDate ?? new Date().toISOString().slice(0, 10),
      materialCost: costs.materialCost,
      finishedGoodsCost: costs.finishedGoodsCost,
      wipVariance: totalVariance,
      totalVariance,
      status: ProductionVarianceStatus.CALCULATED,
    ...
    try {
      return await this.varianceRepository.save(variance);
    } catch (error: unknown) {
  ...
  async post(
    ...
    if (!variance) {
      throw new NotFoundException('Production variance not found.');
    }
     ...
     */
    if (variance.status === ProductionVarianceStatus.POSTED) {
      return variance;
    }

    const currentCosts = await this.getWipCosts(
      companyId,
      variance.productionOrderId,
    );

    const currentVariance = this.money(
      currentCosts.materialCost - currentCosts.finishedGoodsCost,
    );

    if (Math.abs(currentVariance - Number(variance.wipVariance)) > 0.01) {
      throw new ConflictException(
     ...
     */
    variance.totalVariance = currentVariance;

    if (Math.abs(currentVariance) <= 0.01) {
      variance.status = ProductionVarianceStatus.POSTED;
  ...
  async list(companyId: string, filter: ProductionVarianceFilterDto) {
    const [data, total] = await this.varianceRepository.findAndCount({
      where: {
        companyId,
  ...
  private async autoPostProductionVarianceIfEnabled(
    ...
    if (settings?.autoPostProductionVariance === false) {
      return;
    }
  ...
  private async getWipCosts(
    ...
    const raw = await this.wipPostingRepository
      ...
      .andWhere('posting.production_order_id = :productionOrderId', {
        productionOrderId,
      })
      .setParameters({
        material: WipPostingType.MATERIAL_CONSUMPTION,
        finished: WipPostingType.FINISHED_GOODS_RECEIPT,
      })
      .getRawOne<{
        materialCost: string;
    ...
    return {
      materialCost: this.money(Number(raw?.materialCost ?? 0)),
      finishedGoodsCost: this.money(Number(raw?.finishedGoodsCost ?? 0)),
    };
  }
  ...
  private async getComponentActualCosts(
    ...
    const rows = await this.consumptionLineRepository
      .createQueryBuilder('line')
      .innerJoin('line.consumption', 'consumption')
      .select('line.production_order_component_id', 'componentId')
      .addSelect('COALESCE(SUM(line.total_cost), 0)', 'actualCost')
      .where('consumption.production_order_id = :productionOrderId', {
        productionOrderId,
      })
      .andWhere('consumption.reversed_at IS NULL')
      .groupBy('line.production_order_component_id')
      .getRawMany<{
        componentId: string;
        actualCost: string;
      }>();
    ...
    return new Map(
      rows.map((row) => [row.componentId, Number(row.actualCost)]),
    );
  }
