...
export class PurchaseOrdersService {
  constructor(
    ...
    private readonly orderRepository: Repository<PurchaseOrderEntity>,
  ...
  async create(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      const order = orderRepo.create({
        ...
        poNumber: await this.generatePoNumber(companyId, dto.poDate),
        poDate: dto.poDate,
   ...
   async findAll(
    ...
    const query = this.orderRepository
      ...
      .where('purchaseOrder.company_id = :companyId', { companyId });

    if (filter.search?.trim()) {
      query.andWhere(
        new Brackets((qb) =>
          qb
            .where('purchaseOrder.po_number ILIKE :search', {
              search: `%${filter.search?.trim()}%`,
            })
            .orWhere('purchaseOrder.notes ILIKE :search', {
              search: `%${filter.search?.trim()}%`,
            }),
        ),
      );
    ...
    if (filter.supplierId) {
      query.andWhere('purchaseOrder.supplier_id = :supplierId', {
        supplierId: filter.supplierId,
      });
    }

    if (filter.warehouseId) {
      query.andWhere('purchaseOrder.warehouse_id = :warehouseId', {
    ...
    if (filter.dateFrom) {
      query.andWhere('purchaseOrder.po_date >= :dateFrom', {
        dateFrom: filter.dateFrom,
      });
    }

    if (filter.dateTo) {
      query.andWhere('purchaseOrder.po_date <= :dateTo', {
    ...
    query
      .orderBy(
        sortColumns[filter.sortBy] ?? 'purchaseOrder.created_at',
        filter.sortOrder,
      )
    ...
    if (orders.length > 0) {
      ...
      const ordersWithItems = await this.orderRepository.find({
        where: orderIds.map((id) => ({
          id,
          companyId,
        })),
        relations: {
      ...
      const byId = new Map(
        ordersWithItems.map((order) => [order.id, order]),
      );

      for (let index = 0; index < orders.length; index += 1) {
        const hydrated = byId.get(orders[index].id);

        if (hydrated) {
          orders[index] = hydrated;
        }
      }
    }
  ...
  async findOne(
    ...
    return this.map(await this.getEntity(companyId, id));
  }
  ...
  async update(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (dto.items) {
        this.ensureUniqueItems(dto.items.map((item) => item.itemId));
        await itemRepo.delete({ purchaseOrderId: order.id });
        order.items = await itemRepo.save(
          dto.items.map((line) => {
            const totals = this.calculateLine(
              line.quantity,
              line.unitPrice,
  ...
  async send(
    ...
    const order = await this.getEntity(companyId, id);
    ...
    order.updatedBy = userId;
    return this.map(await this.orderRepository.save(order));
  }
  ...
  private ensureEditable(order: PurchaseOrderEntity): void {
    if (order.status !== PurchaseOrderStatus.DRAFT)
      throw new ConflictException('Only draft purchase orders can be edited');
  }
  private ensureUniqueItems(ids: string[]): void {
  ...
  private calculateLine(
    ...
    const taxable = this.round(lineSubtotal - discountAmount);
    const taxAmount = this.round(taxable * (taxPercent / 100));
    return {
      lineSubtotal,
      discountAmount,
      taxAmount,
  ...
  private calculateOrderTotals(order: PurchaseOrderEntity): void {
    order.subtotal = this.round(
      order.items.reduce((s, i) => s + Number(i.lineSubtotal), 0),
    );
    order.discountTotal = this.round(
      order.items.reduce((s, i) => s + Number(i.discountAmount), 0),
    );
    order.taxTotal = this.round(
      order.items.reduce((s, i) => s + Number(i.taxAmount), 0),
    );
  ...
  private async generatePoNumber(
    ...
    const year = new Date(poDate).getUTCFullYear();
    const prefix = `PO-${year}-`;
    const latest = await this.orderRepository
      .createQueryBuilder('purchaseOrder')
      .withDeleted()
      .select('purchaseOrder.po_number', 'poNumber')
      .where('purchaseOrder.company_id = :companyId', { companyId })
      .andWhere('purchaseOrder.po_number LIKE :prefix', {
        prefix: `${prefix}%`,
      })
      .orderBy('purchaseOrder.po_number', 'DESC')
      .getRawOne<{ poNumber?: string }>();
    const current = latest?.poNumber
      ? Number(latest.poNumber.replace(prefix, ''))
      : 0;
  ...
  private mapItem(item: PurchaseOrderItemEntity): PurchaseOrderItemResponseDto {
    return {
      id: item.id,
      itemId: item.itemId,
  ...
  private map(order: PurchaseOrderEntity): PurchaseOrderResponseDto {
    return {
      id: order.id,
