...
describe('QualityManagementService', () => {
  ...
  const inspectionRepository = {
    findOne: jest.fn(),
    save: jest.fn(async (value: QualityInspectionEntity) => value),
    createQueryBuilder: jest.fn(),
  };

  const checkRepository = {
    save: jest.fn(async (value: QualityInspectionCheckEntity) => value),
  };
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getRepositoryToken(QualityInspectionEntity),
          useValue: inspectionRepository,
        },
        {
          provide: getRepositoryToken(QualityInspectionCheckEntity),
          useValue: checkRepository,
        },
        {
          provide: DataSource,
          useValue: {
    ...
    service = module.get(QualityManagementService);
  });
  ...
  it('requires sourceId for production inspections', async () => {
    await expect(
      service.create('company', 'user', {
        ...
        sourceType: QualityInspectionSourceType.PRODUCTION,
  ...
  it('prevents duplicate check names', async () => {
    await expect(
      service.create('company', 'user', {
        inspectionDate: '2026-08-07',
        sourceType: QualityInspectionSourceType.MANUAL,
        inspectedQuantity: 10,
        checks: [
          {
            checkName: 'Visual',
  ...
  it('does not complete while mandatory checks are pending', async () => {
    ...
    await expect(
      service.complete('company', 'user', 'inspection', {
        acceptedQuantity: 10,
        rejectedQuantity: 0,
      }),
