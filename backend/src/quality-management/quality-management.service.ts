...
export class QualityManagementService {
  constructor(
    ...
    private readonly inspectionRepository: Repository<QualityInspectionEntity>,
    @InjectRepository(QualityInspectionCheckEntity)
    private readonly checkRepository: Repository<QualityInspectionCheckEntity>,
  ...
  async create(
    ...
    this.ensureUniqueCheckNames(dto.checks.map((check) => check.checkName));

    return this.dataSource.transaction(async (manager) => {
      const inspectionRepository = manager.getRepository(
        QualityInspectionEntity,
      );
      const checkRepository = manager.getRepository(
        QualityInspectionCheckEntity,
      );

      const inspection = inspectionRepository.create({
        companyId,
        inspectionNumber: await this.generateNumber(
          companyId,
          dto.inspectionDate,
          inspectionRepository,
        ),
        inspectionDate: dto.inspectionDate.slice(0, 10),
      ...
      const checks = dto.checks.map((check) =>
        checkRepository.create({
          inspectionId: savedInspection.id,
          checkName: check.checkName.trim(),
          specification: this.optional(check.specification),
          minimumValue: check.minimumValue ?? null,
          maximumValue: check.maximumValue ?? null,
          actualNumericValue: null,
          expectedText: this.optional(check.expectedText),
          actualText: null,
          result: QualityCheckResult.PENDING,
          isMandatory: check.isMandatory ?? true,
          remarks: this.optional(check.remarks),
        }),
      );

      this.validateCheckDefinitions(checks);

      savedInspection.checks = await checkRepository.save(checks);

      return this.toResponse(savedInspection);
    });
  }
  ...
  async findAll(
    ...
    const qb = this.inspectionRepository
      ...
      .leftJoinAndSelect('inspection.checks', 'check')
    ...
    if (query.sourceType) {
      qb.andWhere('inspection.source_type = :sourceType', {
        sourceType: query.sourceType,
      });
    }

    if (query.sourceId) {
      qb.andWhere('inspection.source_id = :sourceId', {
        sourceId: query.sourceId,
      });
    }

    if (query.itemId) {
      qb.andWhere('inspection.item_id = :itemId', { itemId: query.itemId });
    }

    if (query.warehouseId) {
      qb.andWhere('inspection.warehouse_id = :warehouseId', {
        warehouseId: query.warehouseId,
      });
    }

    if (query.dateFrom) {
      qb.andWhere('inspection.inspection_date >= :dateFrom', {
        dateFrom: query.dateFrom.slice(0, 10),
      });
    }

    if (query.dateTo) {
      qb.andWhere('inspection.inspection_date <= :dateTo', {
        dateTo: query.dateTo.slice(0, 10),
      });
    }

    const [data, total] = await qb
      .orderBy('inspection.inspection_date', 'DESC')
      .addOrderBy('inspection.created_at', 'DESC')
  ...
  async update(
    ...
    const inspection = await this.getEntity(companyId, id);

    this.ensureEditable(inspection);

    if (dto.sourceType !== undefined || dto.sourceId !== undefined) {
      const sourceType = dto.sourceType ?? inspection.sourceType;
      const sourceId = dto.sourceId ?? inspection.sourceId ?? undefined;

      this.validateSource(sourceType, sourceId);

      inspection.sourceType = sourceType;
      inspection.sourceId = sourceId ?? null;
    }

    if (dto.inspectionDate) {
      inspection.inspectionDate = dto.inspectionDate.slice(0, 10);
    }
    if (dto.itemId !== undefined) {
      inspection.itemId = dto.itemId ?? null;
    }
    if (dto.warehouseId !== undefined) {
      inspection.warehouseId = dto.warehouseId ?? null;
    }
    if (dto.inspectedQuantity !== undefined) {
      inspection.inspectedQuantity = dto.inspectedQuantity;
    }
    if (dto.inspectorId !== undefined) {
      inspection.inspectorId = dto.inspectorId ?? null;
    }
    if (dto.notes !== undefined) {
      inspection.notes = this.optional(dto.notes);
    }

    inspection.updatedBy = userId;

    return this.toResponse(await this.inspectionRepository.save(inspection));
  }
  ...
  async start(
    ...
    if (inspection.status !== QualityInspectionStatus.DRAFT) {
      throw new ConflictException('Only draft inspections can be started.');
    }
  ...
  async recordCheckResult(
    ...
    const inspection = await this.getEntity(companyId, inspectionId);
    ...
    const check = inspection.checks.find((row) => row.id === checkId);

    if (!check) {
      throw new NotFoundException('Quality check not found.');
    }

    if (dto.actualNumericValue === undefined && dto.actualText === undefined) {
      throw new BadRequestException('A numeric or text result is required.');
    }

    if (dto.actualNumericValue !== undefined) {
      check.actualNumericValue = dto.actualNumericValue;
    }

    if (dto.actualText !== undefined) {
      check.actualText = this.optional(dto.actualText);
    }

    if (dto.remarks !== undefined) {
      check.remarks = this.optional(dto.remarks);
    }

    check.result = this.evaluateCheck(check);
  ...
  async complete(
    ...
    const pendingMandatory = inspection.checks.some(
      (check) =>
        check.isMandatory && check.result === QualityCheckResult.PENDING,
    );
    ...
    const accepted = Number(dto.acceptedQuantity);
    const rejected = Number(dto.rejectedQuantity);
    const inspected = Number(inspection.inspectedQuantity);
    ...
    const mandatoryFailed = inspection.checks.some(
      (check) => check.isMandatory && check.result === QualityCheckResult.FAIL,
    );

    const failed = mandatoryFailed || rejected > 0;

    if (failed && !dto.failureReason?.trim()) {
      throw new BadRequestException(
        'A failure reason is required for failed inspections.',
      );
    }

    inspection.acceptedQuantity = accepted;
    inspection.rejectedQuantity = rejected;
    inspection.status = failed
      ? QualityInspectionStatus.FAILED
      : QualityInspectionStatus.PASSED;
    inspection.failureReason = failed
      ? (dto.failureReason?.trim() ?? null)
      : null;
  ...
  async getReport(
    ...
    const countByStatus = async (
      ...
      this.inspectionRepository
        ...
        .andWhere('inspection.status = :status', { status })
        .getCount();
    ...
    const [
      ...
      this.inspectionRepository
        ...
        .getCount(),
      countByStatus(QualityInspectionStatus.PASSED),
      countByStatus(QualityInspectionStatus.FAILED),
      countByStatus(QualityInspectionStatus.CANCELLED),
    ]);
    ...
    const totals = await this.inspectionRepository
      .createQueryBuilder('inspection')
    ...
    const completed = passedInspections + failedInspections;
    ...
    return {
      ...
      cancelledInspections,
      openInspections,
      totalInspectedQuantity: inspected,
      totalAcceptedQuantity: accepted,
      totalRejectedQuantity: rejected,
      passRatePercent: this.percent(passedInspections, completed),
  ...
  private evaluateCheck(
    ...
    if (check.actualNumericValue !== null) {
      if (
        check.minimumValue !== null &&
        check.actualNumericValue < check.minimumValue
      ) {
      ...
      if (
        ...
        check.actualNumericValue > check.maximumValue
      ) {
        return QualityCheckResult.FAIL;
      }
    ...
    if (check.actualText !== null) {
      if (check.expectedText !== null) {
        return check.actualText.trim().toLocaleLowerCase() ===
          check.expectedText.trim().toLocaleLowerCase()
  ...
  private validateCheckDefinitions(
    ...
    for (const check of checks) {
      if (
        check.minimumValue !== null &&
        check.maximumValue !== null &&
        check.maximumValue < check.minimumValue
      ) {
        throw new BadRequestException(
  ...
  private validateSource(
    ...
    if (sourceType !== QualityInspectionSourceType.MANUAL && !sourceId) {
      throw new BadRequestException(
  ...
  private ensureUniqueCheckNames(names: string[]): void {
  ...
  private ensureEditable(inspection: QualityInspectionEntity): void {
    if (inspection.status !== QualityInspectionStatus.DRAFT) {
      throw new ConflictException('Only draft inspections can be edited.');
    }
  }
  ...
  private async getEntity(
    ...
    const entity = await this.inspectionRepository.findOne({
      where: { id, companyId },
      relations: {
        checks: true,
      },
  ...
  private async generateNumber(
    ...
    repository: Repository<QualityInspectionEntity>,
  ): Promise<string> {
    const year = new Date(
      `${inspectionDate.slice(0, 10)}T00:00:00.000Z`,
    ).getUTCFullYear();
    ...
    const latest = await repository
      .createQueryBuilder('inspection')
      .withDeleted()
      .select('inspection.inspection_number', 'inspectionNumber')
      .where('inspection.company_id = :companyId', { companyId })
      .andWhere('inspection.inspection_number LIKE :prefix', {
        prefix: `${prefix}%`,
      })
    ...
    const current = latest?.inspectionNumber
      ? Number(latest.inspectionNumber.replace(prefix, ''))
      : 0;
  ...
  private toResponse(
    ...
    return {
      ...
      notes: entity.notes,
      checks: (entity.checks ?? []).map((check) => this.toCheckResponse(check)),
  ...
  private toCheckResponse(
    ...
    return {
      ...
      maximumValue: check.maximumValue,
      actualNumericValue: check.actualNumericValue,
