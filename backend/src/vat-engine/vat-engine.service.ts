...
export class VatEngineService {
  ...
  async create(
    ...
    if (duplicate)
      throw new ConflictException(
        'A VAT return already exists for this exact period',
      );
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      const outputReturns = await this.query(
        manager,
        'sales_return_items',
        'sales_returns',
        'sales_return_id',
      ...
      const input = await this.query(
        manager,
        'purchase_invoice_items',
        'purchase_invoices',
        'purchase_invoice_id',
        'invoice_date',
        "('Posted','PartiallyPaid','Paid')",
        companyId,
        dto.periodStart,
        dto.periodEnd,
      );
      const inputReturns = await this.query(
        manager,
      ...
      const outputTax = round2(
        lines
          .filter((line) => line.direction === VatDirection.OUTPUT)
          .reduce((sum, line) => sum + line.taxAmount, 0),
      );
      const inputTax = round2(
        lines
          .filter((line) => line.direction === VatDirection.INPUT)
          .reduce((sum, line) => sum + line.taxAmount, 0),
      );
      return manager.save(
        manager.create(VatReturnEntity, {
          companyId,
          periodStart: dto.periodStart,
          periodEnd: dto.periodEnd,
  ...
  private aggregate(
    manager: EntityManager,
    positive: VatAggregateRow[],
    negative: VatAggregateRow[],
    ...
    return [...map.entries()].map(([taxPercent, value]) =>
      manager.create(VatReturnLineEntity, {
        direction,
        taxPercent,
  ...
  private add(
    map: Map<number, { base: number; tax: number }>,
    row: VatAggregateRow,
    sign: 1 | -1,
  ): void {
  ...
  async findOne(id: string, companyId: string): Promise<VatReturnEntity> {
    const item = await this.returns.findOne({
      where: { id, companyId },
  ...
  async finalize(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      const item = await repository
        .createQueryBuilder('vatReturn')
        .setLock('pessimistic_write')
        .where('vatReturn.id = :id AND vatReturn.companyId = :companyId', {
          id,
          companyId,
        })
        .getOne();
      if (!item) throw new NotFoundException('VAT return not found');
      if (item.status !== VatReturnStatus.DRAFT)
        throw new BadRequestException(
          'Only draft VAT returns can be finalized',
        );
