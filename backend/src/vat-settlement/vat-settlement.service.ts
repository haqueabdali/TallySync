...
export class VatSettlementService {
  constructor(
    ...
    @InjectRepository(VatSettlementSettingsEntity)
    private readonly settingsRepository: Repository<VatSettlementSettingsEntity>,
    @InjectRepository(VatSettlementEntity)
    private readonly settlementRepository: Repository<VatSettlementEntity>,
    @InjectRepository(AccountEntity)
    private readonly accountRepository: Repository<AccountEntity>,
    @InjectRepository(VatReturnEntity)
    private readonly vatReturnRepository: Repository<VatReturnEntity>,
  ...
  async getSettings(companyId: string): Promise<VatSettlementSettingsEntity> {
    ...
    if (!settings)
      throw new NotFoundException(
        'VAT settlement settings are not configured.',
      );
    return settings;
  }
  ...
  async upsertSettings(
    companyId: string,
    userId: string,
    dto: UpsertVatSettlementSettingsDto,
  ): Promise<VatSettlementSettingsEntity> {
    ...
    const entity =
      existing ??
      this.settingsRepository.create({ companyId, createdBy: userId });
  ...
  async create(
    companyId: string,
    userId: string,
    dto: CreateVatSettlementDto,
  ): Promise<VatSettlementEntity> {
    const vatReturn = await this.vatReturnRepository.findOne({
      where: { id: dto.vatReturnId, companyId },
    });
    if (!vatReturn) throw new NotFoundException('VAT return not found.');
    if (vatReturn.status !== VatReturnStatus.FINALIZED)
      throw new BadRequestException(
        'Only finalized VAT returns can be settled.',
      );
    if (dto.settlementDate < vatReturn.periodEnd)
      throw new BadRequestException(
        'Settlement date cannot be before the VAT return period end.',
      );
    const duplicate = await this.settlementRepository.findOne({
      where: { companyId, vatReturnId: vatReturn.id },
    });
    if (duplicate)
      throw new ConflictException(
        'This VAT return already has a settlement record.',
      );
  ...
  async post(
    ...
    if (settlement.vatReturn.status !== VatReturnStatus.FINALIZED)
      throw new BadRequestException('The related VAT return is not finalized.');
    if (
      this.round(settlement.outputTax) !==
        this.round(settlement.vatReturn.outputTax) ||
      this.round(settlement.inputTax) !==
        this.round(settlement.vatReturn.inputTax) ||
      this.round(settlement.netTaxPayable) !==
        this.round(settlement.vatReturn.netTaxPayable)
    ) {
      throw new ConflictException(
        'VAT return totals changed after settlement creation. Recreate the settlement.',
      );
    }
  ...
  async findAll(companyId: string, filter: VatSettlementFilterDto) {
    ...
    if (filter.dateFrom)
      query.andWhere('settlement.settlement_date >= :dateFrom', {
        dateFrom: filter.dateFrom,
      });
    if (filter.dateTo)
      query.andWhere('settlement.settlement_date <= :dateTo', {
  ...
  private buildJournalLines(
    settlement: VatSettlementEntity,
    settings: VatSettlementSettingsEntity,
  ) {
    ...
    if (outputTax > 0)
      lines.push({
        accountId: settings.outputVatAccountId,
        debit: outputTax,
        credit: 0,
        description: 'Clear output VAT for finalized return',
      });
    if (inputTax > 0)
      lines.push({
        accountId: settings.inputVatAccountId,
        debit: 0,
        credit: inputTax,
        description: 'Clear input VAT for finalized return',
      });
    if (net > 0)
      lines.push({
        ...
        debit: 0,
        credit: net,
        description: 'Recognize VAT payable to tax authority',
      });
    if (net < 0)
      lines.push({
        accountId: settings.vatReceivableAccountId,
        debit: Math.abs(net),
        credit: 0,
        description: 'Recognize VAT recoverable from tax authority',
      });
    ...
    if (debit !== credit)
      throw new BadRequestException(
        `VAT settlement journal is not balanced: debit ${debit}, credit ${credit}.`,
      );
    return lines;
  }
  ...
  private async validateAccounts(
    ...
    const accountIds = [
      ...
      dto.vatReceivableAccountId,
    ];
    if (new Set(accountIds).size !== accountIds.length)
      throw new BadRequestException(
        'VAT settlement accounts must be distinct.',
      );
    ...
    this.assertAccount(
      byId.get(dto.outputVatAccountId),
      companyId,
      AccountType.LIABILITY,
      'Output VAT',
    );
    this.assertAccount(
      byId.get(dto.inputVatAccountId),
      companyId,
      AccountType.ASSET,
      'Input VAT',
    );
    this.assertAccount(
      byId.get(dto.vatPayableAccountId),
      companyId,
      AccountType.LIABILITY,
      'VAT payable',
    );
    this.assertAccount(
      byId.get(dto.vatReceivableAccountId),
      companyId,
      AccountType.ASSET,
      'VAT receivable',
    );
  ...
  private assertAccount(
    ...
    if (!account || account.companyId !== companyId)
      throw new BadRequestException(
        `${label} account was not found for this company.`,
      );
    if (account.type !== type)
      throw new BadRequestException(
        `${label} account must be an ${type} account.`,
      );
