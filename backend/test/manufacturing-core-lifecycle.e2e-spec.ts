...
describe('Manufacturing Core Lifecycle (e2e)', () => {
  ...
  afterAll(async () => {
    if (db && companyId) {
      const cleanup: Array<[string, unknown[]]> = [
        ...
        [
          'DELETE FROM manufacturing_wip_postings WHERE company_id = $1',
          [companyId],
        ],
        [
        ...
        [
          'DELETE FROM manufacturing_wip_accounting_settings WHERE company_id = $1',
          [companyId],
        ],
  ...
  it('runs DRAFT -> RELEASED -> IN_PROGRESS -> COMPLETED with retry-safe accounting and variance', async () => {
    ...
    const assetType =
      accountTypes
        .map((r) => r.value)
        .find((v) => v.toLowerCase() === 'asset') ?? accountTypes[0]?.value;
    const expenseType =
      accountTypes
        .map((r) => r.value)
        .find((v) => v.toLowerCase() === 'expense') ?? assetType;
    const debitBalance =
      normalBalances
        .map((r) => r.value)
        .find((v) => v.toLowerCase() === 'debit') ?? normalBalances[0]?.value;
    ...
    await expect(
      productionVarianceService.calculate(companyId, actorId, orderId, {}),
