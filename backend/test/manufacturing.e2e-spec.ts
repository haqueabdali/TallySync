...
const routes = {
  ...
  releaseProductionOrder: {
    method: 'POST',
    path: '/production-orders/:id/release',
    handler: 'release',
  },
  startProductionOrder: {
    method: 'POST',
    path: '/production-orders/:id/start',
    handler: 'start',
  },
  completeProductionOrder: {
    method: 'POST',
    path: '/production-schedules/:id/complete',
    handler: 'complete',
  },
  ...
  runMrp: {
    method: 'GET',
    path: '/manufacturing/mrp/plan',
    handler: 'getPlan',
  },
  productionVariance: {
    method: 'GET',
    path: '/production-variances/settings',
    handler: 'getSettings',
  },
} as const;
...
describe('Manufacturing Order-to-Completion (e2e)', () => {
  ...
  it.todo('runs MRP twice without changing raw-material currentStock');

  it.todo('starts the production order through a valid lifecycle transition');
  ...
  it.todo('retries completion without duplicating finished-goods stock');

  it.todo('rejects cross-company BOM/component/production references');

  it.todo('calculates production variance deterministically');

  it.todo('reconciles actual production cost against the completed quantity');
});
